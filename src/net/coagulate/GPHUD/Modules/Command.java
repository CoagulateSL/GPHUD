...
public abstract class Command {
	 ...
	 */
	@Nonnull
	public final Response run(@Nonnull final State state,@Nonnull final String[] args) {
	 ...
	 */
	public final Response run(@Nonnull final State state,@Nonnull final SafeMap parameterMap) {
		...
		for (final Argument arg: getArguments()) {
			...
			if (v!=null&&v.length()>getMaximumLength(arg)) {
				return new ErrorResponse(
						arg.name()+" is "+v.length()+" characters long and must be no more than "+getMaximumLength(arg)+
						".  Input has not been processed, please try again");
			}
			if (arg.type()==ArgumentType.BOOLEAN&&v==null) {
				v="false";
			}
			if (v==null) {
	...
	protected final Object convertArgument(final State state,final Argument argument,String v) {
		final ArgumentType type=argument.type();
		switch (type) {
			case TEXT_INTERNAL_NAME:
				if (v.matches(".*[^a-zA-Z\\d].*")) {
					throw new UserInputValidationFilterException(argument.name()+
					                                             " should only consist of alphanumeric characters (a-z 0-9) and you entered '"+
					                                             v+"'",true);
				}
			...
			case TEXT_CLEAN:
				if (v.matches(".*[^a-zA-Z\\d.'\\-, ].*")) {
					throw new UserInputValidationFilterException(argument.name()+
					                                             " should only consist of simple characters (a-z 0-9 .'-,) and you entered '"+
					                                             v+"'",true);
				}
				// don't put anything here, follow up into the next thing
			case TEXT_ONELINE:
			...
			case INTEGER:
				try {
					...
					throw new UserInputValidationParseException(
							"Unable to convert '"+v+"' to a number for argument "+argument.name(),e,true);
				}
			case FLOAT:
				try {
			...
			case MODULE:
				...
				if (m==null) {
					return new UserInputLookupFailureException("Unable to resolve module "+v);
				}
				return m;
			//case FLOAT:
			...
			case ATTRIBUTE:
				...
				for (final Attribute a: state.getAttributes()) {
					if (a.getName().equalsIgnoreCase(v)) {
						...
						if (type==ArgumentType.ATTRIBUTE||a.getSelfModify()) {
							attr=a;
						}
						if (type==ArgumentType.SET&&a.getType()==Attribute.ATTRIBUTETYPE.SET) {
							attr=a;
						}
						if (type==ArgumentType.INVENTORY&&a.getType()==Attribute.ATTRIBUTETYPE.INVENTORY) {
							attr=a;
						}
					}
				}
			...
			case CHARACTER_NEAR:
				...
				if (v.startsWith(">")) {
					...
					try {
						...
						throw new UserInputLookupFailureException("Unable to find character of avatar named '"+v+"'",
						                                          e,
						                                          true);
					}
				} else {
			...
			default:
				throw new SystemImplementationException(
						"Unhandled argument type "+type+" in converter for argument "+argument.name());
		}
	}
	 ...
	 */
	@Nonnull
	public final Response run(@Nonnull final State state,@Nonnull final Map<String,Object> parameterMap) {
		...
		for (final Argument a: getArguments()) {
			...
			if (a.mandatory()&&o==null) {
				return new ErrorResponse(
						"Argument "+a.name()+" is mandatory on command "+getFullName()+" and nothing was passed");
			}
		}
	...
	private void checkCallingInterface(final State state) {
		...
		if (state.source==Sources.SYSTEM) {
			if (!permitHUD()) {
				throw new UserAccessDeniedException(
						getFullName()+" command can not be accessed via the LSL System interface",true);
			}
		}
		...
		if (state.source==Sources.EXTERNAL) {
			if (!permitExternal()) {
				throw new UserAccessDeniedException(
						getFullName()+" command can not be accessed via the External API interface",true);
			}
		}
	...
	private void checkCallingContext(final State state) {
		switch (context()) {
			...
			case CHARACTER:
				if (state.getInstanceNullable()==null) {
					throw new UserInputStateException(
							"Character context required and you are not connected to an instance.",
							true);
				}
			...
			case AVATAR:
				if (state.getInstanceNullable()==null) {
					throw new UserInputStateException(
							"Avatar context required and you are not connected to an instance.",
							true);
				}
	...
	// ----- Internal Instance -----
	protected abstract Response execute(State state,Map<String,Object> arguments);
	 ...
	 */
	public final void simpleHtml(@Nonnull final State st,@Nonnull final SafeMap values) {
	...
	public final JSONObject getJSONTemplate(@Nonnull final State st) {
		...
		for (final Argument argument: getArguments()) {
			...
			switch (argument.type()) {
				...
				default:
					throw new SystemImplementationException(
							"Unhandled ENUM TYPE in getJSONTemplate():"+argument.type());
			}
			arg++;
		}
		json.put("args",arg);
