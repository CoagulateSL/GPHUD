...
public class Add {
	...
	public static Response character(@Nonnull State st,
	                                 @Nonnull @Argument.Arguments(description="Character to log note against",type = Argument.ArgumentType.CHARACTER)
		                                    Char character,
	                                 @Argument.Arguments(description = "Share note with the user",type = Argument.ArgumentType.BOOLEAN)
	                                        Boolean shared,
	                                 @Nonnull @Argument.Arguments(description="Note to record",type=Argument.ArgumentType.TEXT_ONELINE,max=4096)
	                                        String note)
	{
	...
	public static Response avatar(@Nonnull State st,
	                              @Nonnull @Argument.Arguments(description="Avatar to log note against",type = Argument.ArgumentType.AVATAR)
			                                 User target,
	                              @Argument.Arguments(description = "Share note with the user",type = Argument.ArgumentType.BOOLEAN)
			                                 Boolean shared,
	                              @Nonnull @Argument.Arguments(description="Note to record",type=Argument.ArgumentType.TEXT_ONELINE,max=4096)
