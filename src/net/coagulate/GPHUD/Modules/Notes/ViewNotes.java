...
public class ViewNotes {
	...
	public static void viewNotes(@Nonnull final State st,
		...
		if (!isadmin) {
			...
			if (st.getCharacter()!=targetchar) {
				return;
			} // empty return, the view page continues without this section
		}
	...
	// ----- Internal Statics -----
	@Nonnull
	static Table formatNotes(@Nonnull final State st,@Nonnull final List<Note> notes,final String timezone) {
	...
	public static void viewChar(@Nonnull final State st,final SafeMap values) {
		...
		f.add(formatNotes(st,
		                  AdminNote.get(st.getInstance(),target.getOwner(),target,admin,false),
