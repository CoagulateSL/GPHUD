...
public class GSFunctions {
	...
	public static void renderFunction(@Nonnull final State st,final SafeMap values) {
		...
		if (meta.privileged()) {
			f.add("<p><i>This is a privileged function call</i></p>");
		}
		f.add(meta.notes());
	}
	...
	public static void GSFunctionsIndex(@Nonnull final State st,final SafeMap values) {
		final Form f=st.form();
		final Table table=new Table();
		final Map<String,Map<String,String>> output=new TreeMap<>();
		f.add(table);
		for (final String functionname: net.coagulate.GPHUD.Modules.Scripting.Language.Functions.GSFunctions.getAll()
		                                                                                                    .keySet()) {
			String category="";
			String line="";
			try {
				final Method function=
						net.coagulate.GPHUD.Modules.Scripting.Language.Functions.GSFunctions.get(functionname);
				final net.coagulate.GPHUD.Modules.Scripting.Language.Functions.GSFunctions.GSFunction meta=
			...
			if (!output.containsKey(category)) {
				output.put(category,new TreeMap<>());
			}
			output.get(category).put(functionname,line);
		}
		for (final Entry<String,Map<String,String>> catset: output.entrySet()) {
			table.add(new Row(
					"<td colspan=9999><table width=100%><tr width=100%><td width=50%><hr></td><td><span style=\"display: inline-block; white-space: nowrap;\"><b>"+
					catset.getKey()+"</b></span"+"></td><td width=50%><hr></td></tr></table></td></tr><tr>"));
