...
public abstract class EventsMaintenance {
    ...
    public static void maintenance() {
        ...
        for (EventSchedule schedule:events) {
            ...
            State temp=new State(); temp.setInstance(e.getInstance());
    ...
    public static void zoneTransition(State st, JSONObject response, Zone oldzone, Zone zone) {
        ...
        if (debug) { System.out.println("From "+oldzone.getName()+" to "+zone.getName()+" checking "+events.size()); }
        ...
        if (wasin!=nowin) { //transitioned
            if (wasin!=null) {
                ...
                if (exitmessage!=null && !exitmessage.isEmpty()) { response.put("eventmessage1","[Event:"+wasin.getEvent().getName()+"] "+exitmessage); }
