...
public abstract class Login {
	...
	public static Response create(@Nonnull final State st,
		...
		if (charactername==null) {
			return new ErrorResponse("You must enter a name for the new character");
		}
		...
		if (maxchars<=Char.getCharacters(st.getInstance(),st.getAvatar()).size()&&
		    !st.hasPermission("Characters.ExceedCharLimits")) {
	...
	public static Response select(@Nonnull final State st,
		...
		if (character==null) {
			return new ErrorResponse("No such character");
		}
	...
	public static Response initialise(@Nonnull final State st,
		...
		switch (attribute.getType()) {
			case FLOAT,INTEGER,TEXT -> {
				...
				if (attribute.getType()==Attribute.ATTRIBUTETYPE.FLOAT||
				    attribute.getType()==Attribute.ATTRIBUTETYPE.INTEGER) {
						...
						try {
							if (Float.parseFloat(value)>max) {
								...
								JSONResponse.message(json,
								                     "Character creation requires you to input attribute "+
								                     attribute.getName()+" WHICH MUST BE NO MORE THAN "+max,
								                     st.protocol);
								...
								json.put("arg0description",
								         "You must select a "+attribute.getName()+
								         " for your Character before you can use it (no greater than "+max+")");
			...
			case GROUP -> {
				...
				if (attribute.getSubType()!=null) {
					...
					if (group!=null) {
						return new ErrorResponse(
								"You already have membership of '"+group.getNameSafe()+"' which is of type "+
				...
				if (!targettype.equals(attribute.getSubType())) {
					return new ErrorResponse(
							"Group "+target.getNameSafe()+" is of type "+target.getType()+" rather than the required "+
							attribute.getSubType()+" required by "+"attribute "+attribute.getName());
				}
				// check the group is open
				if (!target.isOpen()) {
					return new ErrorResponse(
							"You can not join group "+target.getNameSafe()+" of type "+target.getType()+
							", it is not open for joining");
				}
