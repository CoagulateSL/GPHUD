...
public class AttributeConfig {
	...
	public static Response createAttribute(@Nonnull final State st,
	                                       ...
	                                       @Arguments(description="Allow users to edit their own value without needing permissions", type=Argument.ArgumentType.BOOLEAN) final Boolean selfmodify,
	                                       @Nullable @Arguments(description="Type of this attribute", type=Argument.ArgumentType.CHOICE, choiceMethod="getAttributeTypes") final String attributetype,
	                                       @Nullable @Arguments(description="Type of group, if GROUP, or awarding group TYPE if EXPERIENCE", mandatory=false, type=Argument.ArgumentType.TEXT_INTERNAL_NAME, max=128) final String grouptype,
	                                       @Arguments(description="Increases based off allocation of ability points? (only for INTEGER/FLOAT types)", mandatory=false, type=Argument.ArgumentType.BOOLEAN) final Boolean usesabilitypoints,
	                                       @Arguments(description="Attribute must be completed", type=Argument.ArgumentType.BOOLEAN) final Boolean required,
	                                       @Arguments(description="Default value (can be blank)", type=Argument.ArgumentType.TEXT_ONELINE, max=4096) final String defaultvalue)
	{
		for (final Attribute a: st.getAttributes()) {
		...
		st.getInstance()
		  .createAttribute(name,selfmodify,attributetype,grouptype,usesabilitypoints,required,defaultvalue);
		Audit.audit(st,
	...
	public static Response setSelfModify(@Nonnull final State st,
	                                     @Nonnull @Arguments(description="Attribute", type=Argument.ArgumentType.ATTRIBUTE) final Attribute attribute,
	                                     @Arguments(description="Free self modification allowed?", mandatory=false, type=Argument.ArgumentType.BOOLEAN) final Boolean selfmodify)
	{
		attribute.validate(st);
	...
	public static Response deleteAttribute(@Nonnull final State st,
	                                       @Nonnull @Arguments(description="Attribute **AND ALL ITS DATA** to delete", type=Argument.ArgumentType.ATTRIBUTE) final Attribute attribute,
	                                       @Arguments(description="CONFIRM AS THIS WILL IRREVERSIBLY DELETE DATA ATTACHED TO THIS ATTRIBUTE", mandatory=false, type=Argument.ArgumentType.BOOLEAN) final Boolean confirm)
	{
		attribute.validate(st);
