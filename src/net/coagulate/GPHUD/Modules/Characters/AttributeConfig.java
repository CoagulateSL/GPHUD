...
public class AttributeConfig {
		...
		for (final Attribute a: st.getAttributes()) {
			...
			if (!a.readOnly()&&st.hasPermission("Characters.CreateAttribute")) {
				at.add(new Form(st,
				                true,
				                "./characters/toggleap",
				                String.valueOf(a.usesAbilityPoints()),
				                "attribute",
				                a.getName(),
				                a.usesAbilityPoints()?"usesabilitypoints":"noop",
				                "set"));
			} else {
				at.add(a.usesAbilityPoints());
			}
			
			if (!a.readOnly()&&st.hasPermission("Characters.CreateAttribute")) {
				at.add(new Form(st,
				                true,
				                "./characters/toggleselfmodify",
				                String.valueOf(a.getSelfModify()),
				                "attribute",
				                a.getName(),
				                a.getSelfModify()?"selfmodify":"noop",
				                "set"));
			} else {
				at.add(a.getSelfModify());
			}
			
			if (!a.readOnly()&&st.hasPermission("Characters.CreateAttribute")) {
				at.add(new Form(st,
				                true,
				                "./characters/togglerequired",
				                String.valueOf(a.getRequired()),
				                "attribute",
				                a.getName(),
				                a.getRequired()?"required":"noop",
				                "set"));
			} else {
				at.add(a.getRequired());
			}
			
			if (!a.readOnly()&&st.hasPermission("Characters.CreateAttribute")) {
				at.add(new Form(st,
				                true,
				                "./characters/setdefaultvalue",
				                a.getDefaultValue(),
				                "attribute",
				                a.getName(),
				                "defaultvalue",
				...
				at.add(blankNulls(a.getDefaultValue()));
			}
			
			if (!a.readOnly()&&st.hasPermission("Characters.CreateAttribute")) {
				at.add(new Form(st,
				                true,
				                "./characters/toggletemplatable",
				                String.valueOf(a.templatable()),
				                "attribute",
				                a.getName(),
				                a.templatable()?"templatable":"noop",
				                "set"));
			} else {
				at.add(a.templatable());
			}
	...
	public static Response createAttribute(@Nonnull final State st,
	                                       ...
	                                       @Arguments(name="selfmodify",
	                                                  description="Allow users to edit their own value without needing permissions",
	                                                  type=Argument.ArgumentType.BOOLEAN) final Boolean selfmodify,
	                                       @Nullable
	                                       @Arguments(name="attributetype",
	                                                  description="Type of this attribute",
	                                                  type=Argument.ArgumentType.CHOICE,
	                                                  choiceMethod="net.coagulate.GPHUD.Modules.Characters.AttributeConfig.getAttributeTypes")
	                                       final String attributetype,
	                                       @Nullable
	                                       @Arguments(name="grouptype",
	                                                  description="Type of group, if GROUP, or awarding group TYPE if EXPERIENCE",
	                                                  mandatory=false,
	                                                  type=Argument.ArgumentType.TEXT_INTERNAL_NAME,
	                                                  max=128) final String grouptype,
	                                       @Arguments(name="usesabilitypoints",
	                                                  description="Increases based off allocation of ability points? (only for INTEGER/FLOAT types)",
	                                                  mandatory=false,
	                                                  type=Argument.ArgumentType.BOOLEAN)
	                                       final Boolean usesabilitypoints,
	                                       @Arguments(name="required",
	                                                  description="Attribute must be completed",
	                                                  type=Argument.ArgumentType.BOOLEAN) final Boolean required,
	                                       @Arguments(name="defaultvalue",
	                                                  description="Default value (can be blank)",
		...
		for (final Attribute a: st.getAttributes()) {
			if (a.getName().equalsIgnoreCase(name)) {
				return new ErrorResponse("This name is already claimed");
			}
		}
		...
		for (final String templatename: st.getTemplates().keySet()) {
			if (templatename.equalsIgnoreCase("--"+name+"--")) {
				return new ErrorResponse(
						"There is already a template with that name, it can not also be used as an attribute name");
			}
		}
		if (name.length()<2) {
			return new ErrorResponse("Please enter a longer name");
		}
		...
		if ("GROUP".equals(attributetype)) {
			if (grouptype==null) {
				return new ErrorResponse("Group data type must have a group type attached");
			}
		...
		Attribute.create(st,
		                 name,
		                 selfmodify,
		                 Attribute.fromString(attributetype),
		                 grouptype,
		                 usesabilitypoints,
		                 required,
		                 defaultvalue);
		Audit.audit(st,
		            ...
		            null,
		            null,
		            "Avatar created new attribute "+name+" selfmod:"+selfmodify+" type:"+attributetype+" group:"+
		            grouptype+" AP:"+usesabilitypoints+" REQ:"+required+" "+"Default:"+defaultvalue);
	...
	public static Response setAP(@Nonnull final State st,
		...
		Audit.audit(st,
		            ...
		            String.valueOf(usesabilitypoints),
		            "Avatar set use of ability points");
	...
	public static Response setSelfModify(@Nonnull final State st,
	                                     ...
	                                     @Arguments(name="attribute",
	                                                ...
	                                                type=Argument.ArgumentType.ATTRIBUTE) final Attribute attribute,
	                                     @Arguments(name="selfmodify",
	                                                description="Free self modification allowed?",
	                                                mandatory=false,
	                                                type=Argument.ArgumentType.BOOLEAN) final Boolean selfmodify) {
		attribute.validate(st);
		...
		Audit.audit(st,
		            ...
		            null,
		            null,
		            "Attribute/SetSelfModify",
		            attribute.getName(),
		            String.valueOf(oldvalue),
		            String.valueOf(selfmodify),
		            "Avatar set self modify");
	...
	public static Response deleteAttribute(@Nonnull final State st,
	                                       @Nonnull
	                                       @Arguments(name="attribute",
	                                                  description="Attribute **AND ALL ITS DATA** to delete",
	                                                  type=Argument.ArgumentType.ATTRIBUTE) final Attribute attribute,
	                                       @Arguments(name="confirm",
	                                                  description="CONFIRM AS THIS WILL IRREVERSIBLY DELETE DATA ATTACHED TO THIS ATTRIBUTE",
	                                                  mandatory=false,
	                                                  type=Argument.ArgumentType.BOOLEAN) final Boolean confirm) {
		attribute.validate(st);
		if (!confirm) {
			return new ErrorResponse("You did not confirm the operation");
		}
		attribute.delete(st);
		Audit.audit(st,
		            Audit.OPERATOR.AVATAR,
		            ...
		            null,
		            null,
		            "Avatar DELETED ATTRIBUTE ENTIRELY");
		return new OKResponse("Attribute and attached data has been DELETED");
	}
	
	@URLs(url="/configuration/characters/toggletemplatable", requiresPermission="Characters.CreateAttribute")
	...
	public static Response setTemplatable(@Nonnull final State st,
	                                      ...
	                                      @Arguments(name="attribute",
	                                                 description="Attribute",
	                                                 type=Argument.ArgumentType.ATTRIBUTE) final Attribute attribute,
	                                      @Arguments(name="templatable",
	                                                 description="Does templating?",
	                                                 mandatory=false,
