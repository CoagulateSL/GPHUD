...
public class CharacterList {
	...
	public static void list(@Nonnull final State st,final SafeMap parameters) {
		listInternal(st,parameters,false);
	}
	
	// ----- Internal Statics -----
	private static void listInternal(@Nonnull final State st,final SafeMap parameters,final boolean showretired) {
		
		int page=0;
		String searchtext="";
		
		if (parameters.containsKey("page-default")) {
			page=Integer.parseInt(parameters.get("page-default"));
		}
	...
	private static String getPageRow(final int page,final int rowcount,final String searchtext) {
		String r="";
		final int pagecount=(int)Math.ceil(((double)rowcount)/50);
		r+="<tr><td align=left colspan=99999>";
		r+="<button type=submit name=submit value=submit>Search:</button><input type=text name=search value=\""+
		   searchtext+"\">";
		r+="&nbsp;&nbsp;&nbsp;";
	...
	@URLs(url="/characters/retiredlist*", requiresPermission="Characters.ViewAll")
	public static void listWithRetired(@Nonnull final State st,final SafeMap parameters) {
		listInternal(st,parameters,true);
	}
