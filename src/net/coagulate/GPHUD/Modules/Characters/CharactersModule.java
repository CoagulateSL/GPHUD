...
public class CharactersModule extends ModuleAnnotation {
    ...
    public static String templateAttribute(State st,String template) {
        ...
        for (Attribute a:st.getAttributes()) {
            if (a.getName().equalsIgnoreCase(template)) {
                if (attr!=null) { throw new SystemException("Unexpected duplicate resolution for attribute "+attr.getName()+" and "+a.getName()); }
                attr=a;
            }
        }
    ...
    public static Response spendAbilityPoint(State st,
        ...
        if (!attr.usesAbilityPoints()) { return new ErrorResponse("This attribute can not be increased through ability points"); }
        if (attr.getKVType()!=KV.KVTYPE.FLOAT && attr.getKVType()!=KV.KVTYPE.INTEGER) { throw new UserException("Can not increase a non integer/float attribute type"); }
                ...
                String value)
    {
        ...
        if (attribute.getSelfModify()==false) { return new ErrorResponse("This attribute is not self modifiable, it can only be changed directly by an admin"); }
