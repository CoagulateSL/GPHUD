...
public class CommandAnnotation extends Command {
	...
	void validate(final State st) {
		...
		if (method.getParameterCount()==0) {
			throw new SystemImplementationException(
					"Method "+getFullName()+"() takes zero arguments but must take 'State' as its first argument");
		}
		...
		for (int i=1;i<method.getParameters().length;i++) {
			...
			if (arg==null) {
				throw new SystemImplementationException(
						"Method "+getFullName()+"() argument "+(i+1)+" ("+p.getName()+") has no @Argument metadata");
			}
			if (arg.type()==ArgumentType.CHOICE) {
				...
				try {
					...
					throw new SystemImplementationException(
							"Failed to instansiate choice method "+getFullName()+" / "+choicemethod,e);
				}
			}
		}
	...
	private void populateArguments() {
		...
		for (final Parameter p: method.getParameters()) {
			if (skipfirst) {
				...
				arguments.add(new ArgumentAnnotation(this,p));
			}
		}
	}
	...
	protected Response execute(final State state,final Map<String,Object> arguments) {
		try {
			...
			throw new SystemImplementationException(
					"An illegal argument leaked to "+getFullName()+" at "+getMethod().getClass().getName()+"."+
					getMethod().getName(),e);
		} catch (final IllegalAccessException e) {
			throw new SystemImplementationException("Access to target method "+getMethod().getName()+" in "+
			                                        getMethod().getDeclaringClass().getSimpleName()+" denied by JVM",e);
		} catch (final InvocationTargetException e) {
			final Throwable content=e.getCause();
			if (content==null) {
				throw new SystemImplementationException("Null invocation target exception cause",e);
			}
			...
			throw new SystemExecutionException(
					"Annotated command "+getFullName()+" from "+getMethod().getName()+" in class "+
					getMethod().getDeclaringClass().getSimpleName()+" exceptioned: "+content.getLocalizedMessage(),
					content);
		}
	}
	...
	public boolean permitWeb() {
		return meta.permitUserWeb();
	}
	
	public boolean permitHUD() {
		return meta.permitJSON();
	}
	
	public boolean permitConsole() {
		return meta.permitConsole();
	}
	
	public boolean permitScripting() {
		return meta.permitScripting();
	}
	
	public boolean permitExternal() {
		return meta.permitExternal();
	}
	// ----- Internal Instance -----
