...
@Permissions(name = "OWNER", description = "Exclusive to the owner of the instance", grantable = false)
@Permissions(name = "SetMOTD", description = "Allowed to set the instance Message of the Day (MOTD)")
...
@Permissions(name = "ReceiveAdminMessages", description = "These users will receive administrative messages in addition to the instance owner")
@Permissions(name = "SendAdminMessages", description = "These users will be able to send administrative messages")
@KVS(name = "MaxCharacters", description = "Maximum number of characters an avatar might own", defaultvalue = "1", editpermission = "Instance.Owner", hierarchy = KVHIERARCHY.CUMULATIVE, scope = KVSCOPE.COMPLETE, template = false, type = KVTYPE.INTEGER)
@KVS(name = "CharacterSwitchEnabled", description = "Allowed to switch/create characters (consider applying to a zone)", defaultvalue = "false", editpermission = "Instance.Owner", hierarchy = KVHIERARCHY.DELEGATING, scope = KVSCOPE.COMPLETE, template = false, type = KVTYPE.BOOLEAN)
@KVS(name = "AutoNameCharacter", description = "Should avatars have a default character created using their avatar name (traditional behaviour)", editpermission = "Instance.Owner", defaultvalue = "true", hierarchy = KVHIERARCHY.NONE, scope = KVSCOPE.INSTANCE, template = false, type = KVTYPE.BOOLEAN)
