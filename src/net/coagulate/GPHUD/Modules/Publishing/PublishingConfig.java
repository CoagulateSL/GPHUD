...
public class PublishingConfig {
	// ---------- STATICS ----------
	@URL.URLs(url="/configuration/publishing", requiresPermission="Publishing.*")
	public static void configPage(@Nonnull final State st,final SafeMap values) {
		...
		f.add(new Text(
				"<p><i><b>Note:</b> This feature is in early release, please feel free to submit ideas for published pages, layouts, options, formatting "+
				"etc</i></p"+">"));
		f.noForm();
		...
		if (st.getKV("Publishing.PublishGroups").boolValue()) {
			...
			for (final CharacterGroup group: st.getInstance().getCharacterGroups()) {
				...
				if (gt!=null) {
					if (!grouptypes.contains(gt)) {
						grouptypes.add(gt);
					}
				}
			}
			for (final String type: grouptypes) {
				f.add(" <a href=\"/GPHUD/publishing/grouptype/"+st.getInstance().getId()+"/"+type+"\">GroupType:"+type+
				      "</a>");
			}
			f.add("<br>");
		}
		...
		if (!publishedany) {
			f.add("No pages are published ; enable them below to see them listed here");
		}
		f.add(new Separator());
