...
public abstract class Templater {
	...
	public static Method getMethod(final State st,final String name) {
	...
	@Nullable
	private static String template(@Nonnull final State st,@Nullable String string) {
		if (string==null) {
			return null;
		}
			...
			double parse() {
				...
				if (pos<str.length()) {
					throw new UserInputValidationParseException(
							"Unexpected: "+(char)ch+" at position "+pos+" in '"+str+"'",true);
				}
				return x;
			}
				...
				if (eat('(')) { // parentheses
					...
					if ((ch>='0'&&ch<='9')||ch=='.') { // numbers
						...
						if (ch>='a'&&ch<='z') { // functions
							...
							throw new UserInputValidationParseException(
									"Unexpected: "+(char)ch+" at "+pos+" in '"+str+"'",true);
						}
					}
				}
	...
	private static String getValue(@Nonnull final State st,final String keyword) {
		if (Thread.currentThread().getStackTrace().length>150) {
			throw new UserConfigurationException("Recursion detected loading template "+keyword+" for "+st);
		}
		...
		if (m!=null) {
			try {
				...
				if (UserException.class.isAssignableFrom(e.getCause()
				                                          .getClass())) { //noinspection ThrowInsideCatchBlockWhichIgnoresCaughtException
					throw (UserException)e.getCause();
				}
				if (SystemException.class.isAssignableFrom(e.getCause()
				                                            .getClass())) { //noinspection ThrowInsideCatchBlockWhichIgnoresCaughtException
	...
	public static String newline(final State st,final String key) {
		return "\n";
	}
