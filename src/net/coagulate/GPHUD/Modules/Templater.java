...
public abstract class Templater {
    ...
    private static void add(String key,String description,Method method) { templates.put("--"+key+"--",description); methods.put("--"+key+"--",method); }
    ...
    private static String template(State st,String string) throws UserException{
        ...
        for (String subst:getTemplates(st).keySet()) {
            if (string.indexOf(subst)!=-1) { 
                ...
                String value="ERROR";
                try { value=getValue(st,subst); }
                catch (UserException e) { value="Error: "+e.getMessage(); }
    ...
    public static String getAvatarName(State st,String key) { if (st.avatar()==null) { return ""; } return st.avatar().getName(); }
    ...
    public static double eval(final String str) throws UserException {
        return new Object() {
            ...
            double parse() {
                ...
                if (pos < str.length()) throw new UserException("Unexpected: " + (char)ch+" at position "+pos+" in '"+str+"'");
                return x;
            }
