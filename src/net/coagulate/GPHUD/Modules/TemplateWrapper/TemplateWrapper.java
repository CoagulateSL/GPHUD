...
public class TemplateWrapper extends ModuleAnnotation {
	...
	public TemplateWrapper(final String name,final ModuleDefinition annotation) {
		super(name,annotation);
	}
	
	// ---------- STATICS ----------
	@Nullable
	public static String templateWrapper(final State st,@Nonnull final String wrappedtemplate) {
	...
	public void addTemplateMethods(@Nonnull final State st,@Nonnull final Map<String,Method> cumulativeMap) {
		...
		for (final String template: getExternalTemplates(st).keySet()) {
			try {
				cumulativeMap.put("--WRAPPED:"+template.toUpperCase()+"--",
				                  getClass().getMethod("templateWrapper",State.class,String.class));
