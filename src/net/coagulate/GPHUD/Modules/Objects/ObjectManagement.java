...
public class ObjectManagement {
	...
	public static void createObjectType(@Nonnull final State st,@Nonnull final SafeMap map) {
		if ("Create".equalsIgnoreCase(map.get("Create"))&&
		    ObjectType.getObjectTypesSet().contains(map.get("behaviour"))) {
	...
	public static void editObjectType(@Nonnull final State st,final SafeMap map) {
		...
		if (parts.length<5) {
			throw new UserInputValidationParseException("URI misformed, no ID found");
		}
	...
	public static Response deleteObjectType(@Nonnull final State st,
	                                        @Argument.Arguments(name="name",
	                                                            description="Object Type to remove",
	                                                            type=Argument.ArgumentType.TEXT_ONELINE,
	                                                            max=64) final String name) {
		final ObjType ot=ObjType.get(st,name);
		if (ot==null) {
			return new ErrorResponse("Can not delete object type "+name+" - it does not exist");
		}
		ot.delete();
	...
	@URL.URLs(url="/configuration/objects/deleteobjecttype", requiresPermission="Objects.ObjectTypes")
