...
public class ArgumentAnnotation extends Argument {
	...
	public ArgumentAnnotation(final Command c,@Nonnull final Parameter p) {
		...
		if (!choiceMethod().isEmpty()) {
			CommandAnnotation.checkPublicStatic(getMethod());
		}
	}
	
	@Nonnull
	public String choiceMethod() {
		return meta.choiceMethod();
	}
	
	public Method getMethod() {
		return getMethod(choiceMethod());
	}
	
	// ---------- STATICS ----------
	public static Method getMethod(final String fqn) {
		if (!fqn.contains(".")) {
			throw new SystemImplementationException("Non fully qualified method name in "+fqn);
		}
		final Matcher matcher=Pattern.compile("(.*)\\.([^.]*)").matcher(fqn);
		if (!matcher.matches()) {
			throw new SystemImplementationException("Regexp matcher failure for fq method "+fqn);
		}
		if (matcher.groupCount()!=2) {
			throw new SystemImplementationException("Qualified method name "+fqn+" broke into more than 2 parts");
		}
	...
	public List<String> getChoices(final State st) {
		try {
			...
			throw new SystemImplementationException(
					"Access modifier problem loading choices from "+command.getFullName()+"/"+choiceMethod()+"()",ex);
			...
			throw new SystemImplementationException(
					"Security problem loading choices from "+command.getFullName()+"/"+choiceMethod()+"()",ex);
		}
	}
