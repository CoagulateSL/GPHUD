...
public abstract class Management {
    ...
    public static Response setOwner(State st,
        ...
        if (!ingroup) { return new ErrorResponse("New leader "+newowner.getName()+" must be in group "+group.getName()); }
        if (oldowner!=null) { oldowner.hudMessage(("You are no longer the group leader for "+group.getName())); }
    ...
    public static Response add(State st,
        ...
        if (existinggroup!=null && existinggroup.getOwner()==newmember) { return new ErrorResponse("Refusing to move character "+newmember.getName()+", they are currently group leader of "+existinggroup.getName()+", you must manually eject them from that position"); }
        if (existinggroup!=null) { existinggroup.removeMember(newmember); }
        try { group.addMember(newmember); }
        catch (UserException e) { return new ErrorResponse("Failed to add "+newmember.getName()+" to "+group.getName()+", they are probably in no group now! - "+e.getMessage()); }
        String oldgroupname=null; if (existinggroup!=null) { oldgroupname=existinggroup.getName(); }
    ...
    public static Response remove(State st,
            ...
            @Arguments(description = "Character to remove from the group",type = ArgumentType.CHARACTER)
                Char member) {
        if (group.getOwner()==member) { return new ErrorResponse("Will not remove "+member.getName()+" from "+group.getName()+", they are the group leader, you must demote them by replacing them or leaving the group leaderless."); }
    ...
    {
        if (!group.hasMember(character)) { return new ErrorResponse(character.getName()+" is not a member of group "+group.getName()); }
        ...
        if (!ok) { return new ErrorResponse("You must be group owner, or have Groups.Create permissions to set admin flags"); }
