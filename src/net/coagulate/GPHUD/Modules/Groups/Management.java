...
public abstract class Management {
	...
	public static void manage(@Nonnull final State st,
		...
		if (typeFilter!=null) {
			f.add(new TextSubHeader("Filtered by type: "+typeFilter));
		}
	...
	public static void manageType(@Nonnull final State st,final SafeMap values) {
	...
	public static Response create(@Nonnull final State st,
	                              ...
	                              @Arguments(type=ArgumentType.CHOICE,
	                                         ...
	                                         mandatory=false,
	                                         choiceMethod="net.coagulate.GPHUD.Modules.Groups.Management.groupTypes")
	                              final String type) {
		try {
			st.getInstance().createCharacterGroup(name,false,type);
	...
	public static Response setOwner(@Nonnull final State st,
	                                ...
	                                @Arguments(name="group",
	                                           description="Group to change the leader of",
		...
		if (newOwner==oldOwner) {
			if (newOwner==null) {
				return new OKResponse("That group already has no leader");
			}
	...
	@URLs(url="/groups/setprecedence", requiresPermission="Groups.SetPrecedence")
	public static void setPrecedenceForm(@Nonnull final State st,@Nonnull final SafeMap values) {
	...
	public static Response setPrecedence(@Nonnull final State st,
	                                     ...
	                                     @Arguments(name="group",
	                                                ...
	                                                type=ArgumentType.CHARACTERGROUP) final CharacterGroup group,
	                                     @Nonnull
	                                     @Arguments(name="precedence",
	                                                description="New precedence",
		...
		Audit.audit(st,
		            ...
		            "SetPrecedence",
		            group.getName(),
		            String.valueOf(oldPrecedence),
		            String.valueOf(precedence),
		            "Changed KV Precedence from "+oldPrecedence+" to "+precedence);
	...
	public static Response add(@Nonnull final State st,
	                           ...
	                           @Arguments(name="newmember",
	                                      description="Character to add to the group",
	                                      type=ArgumentType.CHARACTER) final Char newMember) {
		return add(st,group,newMember,true);
	}
	
	private static Response add(@Nonnull final State st,
	                            final CharacterGroup group,
	                            final Char newMember,
		...
		try {
			...
			return new ErrorResponse("Failed to add "+newMember.getName()+" to "+group.getName()+
			                         ", they are probably in no group now! - "+e.getMessage());
		}
		String oldGroupName=null;
		if (existingGroup!=null) {
			oldGroupName=existingGroup.getName();
		}
		String result=newMember.getName()+" is now in group "+group.getName();
		if (existingGroup!=null) {
			result=newMember.getName()+" was moved into group "+group.getName()+" (was formerly in "+
	...
	public static Response remove(@Nonnull final State st,
	                              ...
	                              @Arguments(name="member",
	                                         description="Character to remove from the group",
	                                         type=ArgumentType.CHARACTER) final Char member) {
		return remove(st,group,member,true);
	}
	
	private static Response remove(@Nonnull final State st,
	                               final CharacterGroup group,
	                               final Char member,
	                               final boolean notify) {
		if (group.getOwner()==member) {
			return new ErrorResponse("Will not remove "+member.getName()+" from "+group.getName()+
			                         ", they are the group leader, you must demote them by replacing them or "+
	...
	public static Response removeSilently(@Nonnull final State st,
	                                      ...
	                                      @Arguments(type=ArgumentType.CHARACTERGROUP,
	                                                 name="group",
	                                                 description="Group to remove character from")
	                                      final CharacterGroup group,
	                                      @Nonnull
	                                      @Arguments(name="member",
	                                                 description="Character to remove from the group",
	...
	public static Response setAdmin(@Nonnull final State st,
	                                @Nonnull
	                                @Arguments(name="group",
	                                           description="Group to set the character's admin flag on",
	                                           type=ArgumentType.CHARACTERGROUP) final CharacterGroup group,
	                                @Nonnull
	                                ...
	                                @Arguments(name="admin",
	                                           description="Admin flag to set on the character in this group",
	...
	public static Response setOpen(@Nonnull final State st,
	                               ...
	                               @Arguments(name="group",
	                                          ...
	                                          type=ArgumentType.CHARACTERGROUP) final CharacterGroup group,
	                               @Arguments(name="open", description="Group open?", type=ArgumentType.BOOLEAN)
	                               final boolean open) {
		group.validate(st);
		// must be instance admin or group owner
