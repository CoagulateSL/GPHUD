...
public class GroupCommands {
	...
	public static Response join(@Nonnull final State st,
	                            @Arguments(name="group",
	                                       description="Name of group to join",
	                                       type=ArgumentType.CHARACTERGROUP) @Nonnull final CharacterGroup group) {
		
		// is group open?
		if (!group.isOpen()) {
			return new ErrorResponse(
					"Can not join group "+group.getNameSafe()+", it is not open, you will need to be invited instead.");
		}
		...
		if (group.getType()!=null&&!group.getType().isEmpty()) {
			...
			try {
				attribute=Attribute.findGroup(st.getInstance(),group.getType());
			} catch (final TooMuchDataException e) {
				return new ErrorResponse("There is more than one attribute of type GROUP/"+group.getType()+
				                         " which will not work as group subtypes enforce unique membership"+"."+
				                         "  The administrator should remove one of the attributes.");
			}
			if (!attribute.getSelfModify()) {
				return new ErrorResponse(
						"You can not change your "+attribute.getNameSafe()+" after character creation.");
			}
			...
			if (existinggroup!=null) {
				...
				if (remove instanceof ErrorResponse) {
					return new ErrorResponse(
							"Can't switch group - would require you to leave "+existinggroup.getNameSafe()+
							" but this failed: "+((ErrorResponse)remove).getMessage(st));
				}
	...
	public static Response invite(@Nonnull final State st,
		...
		return new OKResponse(
				"Invite message sent to "+target.getName()+" for "+group.getName()+", they have 48 hours to accept.");
	}
	...
	public static Response eject(@Nonnull final State st,
	                             ...
	                             @Arguments(name="member",
	                                        description="Character to eject from the group",
		...
		if (!group.hasMember(member)) {
			return new ErrorResponse(member.getName()+" is not in "+group.getName());
		}
		// refuse if they're the groupleader
		if (group.getOwner()==member) {
			return new ErrorResponse(
					"Will not eject "+member.getName()+" from "+group.getName()+", they are the owner.");
		}
		// refuse if they're an admin.  leader can demote them I hope
		if (group.isAdmin(member)) {
			return new ErrorResponse("Will not eject "+member.getName()+" from "+group.getName()+
			                         ", they are an administrator and must be demoted first.");
		}
		try {
	...
	@Nonnull
	@Commands(context=Context.CHARACTER, description="Leave a group")
	public static Response leave(@Nonnull final State st,
		...
		if (!group.hasMember(st.getCharacter())) {
			return new ErrorResponse("You are not a member of group "+group.getName());
		}
		// group must be open
		if (!group.isOpen()) {
			return new ErrorResponse("You can not leave a non-open group");
		}
		...
		if (!group.getTypeNotNull().isEmpty()) {
			...
			if (!attr.getSelfModify()) {
				return new ErrorResponse(
						"You may not modify the group type "+group.getTypeNotNull()+", it is not self modifiable");
			}
		}
		// is open and not an attribute and is in group so leave...
		try {
			group.removeMember(st.getCharacter());
		} catch (@Nonnull final UserException e) {
			return new ErrorResponse("Failed to leave group - "+e.getMessage());
		}
		Audit.audit(st,
		            OPERATOR.CHARACTER,
		            null,
