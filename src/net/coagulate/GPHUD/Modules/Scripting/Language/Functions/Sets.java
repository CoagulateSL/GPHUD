...
public class Sets {
	...
	public static BCInteger gsSetAdd(@Nonnull final State st,
		...
		Audit.audit(true,
		            ...
		            "Added "+amount.getContent()+" "+element.getContent()+" to set, changing total from "+oldValue+
		            " to "+newAmount);
	...
	private static CharacterSet getSet(final State st,final BCCharacter character,final BCString setName) {
		...
		if (attribute.getType()!=Attribute.ATTRIBUTETYPE.SET) {
			throw new UserInputStateException(
					"Attribute "+attribute.getName()+" is of type "+attribute.getType()+" not SET");
		}
		// Attribute must belong to instance (!)
		if (attribute.getInstance()!=st.getInstance()) {
			throw new SystemImplementationException(
					"Attribute "+attribute+" is not from instance "+st.getInstanceString());
		}
		// check character is of right instance
		if (st.getInstance()!=character.getContent().getInstance()) {
			throw new SystemImplementationException(
					"Target character "+character.getContent()+" is not from instance "+st.getInstanceString());
		}
	...
	public static BCInteger gsSetSet(@Nonnull final State st,
		...
		Audit.audit(true,
		            ...
		            set.getContent(),
		            String.valueOf(oldValue),
		            String.valueOf(amount.getContent()),
		            "Added "+amount.getContent()+" "+element.getContent()+" to set, changing total from "+oldValue+
		            " to "+amount.getContent());
		return amount;
	}
	...
	public static BCInteger gsSetWipe(@Nonnull final State st,
		...
		Audit.audit(true,
		            ...
		            null,
		            null,
		            "Wiped set, formerly containing "+elements+" elements totalling "+totalQuantity+" quantity");
	...
	public static BCInteger gsSetCopy(@Nonnull final State st,
		...
		Audit.audit(true,
		            ...
		            character.getContent(),
		            "gsSetCopy",
		            destinationSet.getContent(),
		            null,
		            null,
		            "Copied set "+sourceSet.getContent()+" to "+destinationSet.getContent()+" which now contains "+
		            totalItems+" totalling quantity "+totalQuantity);
		return new BCInteger(null,totalQuantity);
	}
	...
	@GSFunctions.GSFunction(description="Returns a list of all elements in the set along with their quantity",
	                        ...
	                        parameters="Character character - Character to query<br>"+"String set - Name of set to map",
