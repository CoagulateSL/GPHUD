...
public class Output {
	...
	@Nonnull
	@GSFunctions.GSFunction(description="Causes the HUD to speak in local chat, as the character", parameters="Character - character whose HUD will emit the message<br>String - message to speak", returns="Integer - The number 0", notes="Messages are stacked up, per user, until the script completes or is suspended")
	...
	@Nonnull
	@GSFunctions.GSFunction(description="Causes the HUD to send a message to its wearer", parameters="Character - character whose HUD will message the wearer (the character themselves)<br>String - message to pass", returns="Integer - The number 0", notes="Messages are stacked up, per user, until the script completes or is suspended")
	                                   ...
	                                   @Nonnull final BCString landmark)
	{
		...
		if (!Modules.get(null,"Teleportation").isEnabled(st)) {
			throw new GSResourceUnavailableException(
					"Teleportation module is disabled, thus teleport function calls are disabled.");
		}
		if (!target.isOnline()) { throw new GSResourceUnavailableException("Character "+target+" is not online"); }
