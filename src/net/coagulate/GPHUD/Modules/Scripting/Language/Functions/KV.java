...
public class KV {
	...
	@GSFunctions.GSFunction(description="Returns the value of a KV",
	                        ...
	                        parameters="Character character - The character to look the "+"value up for (use any character if the value doesn't depend on the character)"+
			                        "<br>String kvname - The name of the KV to lookup",
	                        notes="<B>BEWARE:</b> This "+"function <B>ALWAYS</B> returns a String value, even if you're getting a number, so code like <br><pre>Integer "+
			                        "value=gsGetKV(char,\"Characters.stat\")"+"+3</pre"+"><br>will return, for example, 13 if the character's stat is 1, as this is string "+
			                        "addition rules<br>To cast the value to an integer, first do "+"<br><pre"+">Integer value=gsGetKV(char,\"characters.stat\"); Integer "+
			                        "maths=value+3;</pre><bre>the initial value will cast the String to an Integer, which will make"+" the "+"maths in the 2nd line work "+
			                        "properly (i.e. produce 6)",
	                        privileged=false)
