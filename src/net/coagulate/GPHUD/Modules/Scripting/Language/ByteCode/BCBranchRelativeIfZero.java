...
public class BCBranchRelativeIfZero extends ByteCode {
	...
	@Nullable
	private final BCLabel source;
	@Nullable
	private final BCLabel target;

	public BCBranchRelativeIfZero(final ParseNode n,
								  @Nullable final BCLabel source,
	...
	// ---------- INSTANCE ----------
	@Nonnull
	public String explain() { return "BranchRelativeIfZero#"+target().id+" (Pop one, branch relative if zero)"; }
	...
	public void toByteCode(@Nonnull final List<Byte> bytes) {
		...
		else { addInt(bytes,target().address()-source().address()-5); }
		// 5 is the instruction length for BranchRelativeIfZero - 1 byte opcode, 4 bytes relative offset
	}

	@Nullable
	@Override
	public String htmlDecode() { return "BranchRelativeIfZero</td><td>"+target().address(); }
	...
	private BCLabel source() {
		if (source==null) { throw new GSInternalError("Source is null"); }
		return source;
	}
	@Nonnull
