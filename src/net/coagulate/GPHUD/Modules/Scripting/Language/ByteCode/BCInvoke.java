...
public class BCInvoke extends ByteCode {
		...
		if (argcount!=0||parameters.length!=2) {
			...
			if (argcount!=(parameters.length-2)) {
				throw new GSInvalidFunctionCall(
						"Call to "+functionname+" has incorrect number of arguments - it requires "+
						(parameters.length-2)+" and was supplied "+argcount,true);
			}
			...
			for (int i=0;i<argcount;i++) {
				if (!parameters[i+2].isAssignableFrom(args[i].getClass())) {
					throw new GSInvalidFunctionCall(
							...
							args[i].getClass().getSimpleName(),true);
				}
				pass[i+2]=args[i];
			}
			vm.push(invoke(st,vm,function,pass));
	...
	private void executeCall(final State st,
		...
		if (vm.get(" CODEBASE "+scriptname)==null) { // function is not loaded
			...
			if (script.getCompilerVersion()<1) {
				throw new GSInvalidFunctionCall("Script "+scriptname+
				                                " must be recompiled (it was compiled by a compiler that used absolute addressing in branch instructions which inhibits relocation / inclusion).");
			}
	...
	private ByteCodeDataType invoke(final State st,
		...
		try {
			...
			throw new GSInternalError("Method access to "+function.getName()+" in "+function.getDeclaringClass()+
			                          " is not permitted.  Check access qualifier is public.",e);
			...
			if (t!=null) {
				...
				throw new GSInternalError(
						"Unhandled exception in GPHUDScript invoke bytecode calling "+function.getName()+" in "+
						function.getDeclaringClass().getSimpleName(),t);
			}
			throw new GSInternalError(
