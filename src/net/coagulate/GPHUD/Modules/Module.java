...
public abstract class Module {
    ...
    public boolean isEnabled(State st) throws UserException, SystemException {
        ...
        if (!canDisable()) { if (debug) { System.out.println("cant disable so true"); } return true; }
        if (st==null) { if (debug) { System.out.println("No state so true"); }return true; }
        if (!dependanciesEnabled(st)) { if (debug) { System.out.println("dep failure so false"); }return false; }
...
dead code?
    ...
    */
    ...
    public void validateKV(State st,  String key) {
        if (getKVDefinitions(st).containsKey(key.toLowerCase())) {  throw new SystemException("KV does not exist ["+key+"] in ["+this.getName()+"]"); }
    }

    public void validatePermission(State st,String permission) {
        Map<String, Permission> perms = getPermissions(st);
        if (!perms.containsKey(permission.toLowerCase())) {  throw new SystemException("Permission does not exist ["+permission+"] in ["+this.getName()+"]"); }
    }

    public void validateCommand(State st, String command) {
        if (!getCommands(st).containsKey(command.toLowerCase())) {  throw new SystemException("Command does not exist ["+command+"] in ["+this.getName()+"]"); }
    }
