...
public abstract class Groups {
	...
	public static void view(@Nonnull final State st,final SafeMap values) {
		...
		for (final PermissionsGroup.PermissionsGroupMembership member: members) {
			...
			if (member.caninvite) {
				membertable.add(new Color("green","Yes"));
			} else {
			...
			if (st.hasPermission("instance.managepermissions")) {
				...
				if (member.caninvite) {
					spf.add(new Hidden("CanInvite","on"));
				}
				if (member.cankick) {
					spf.add(new Hidden("CanKick","on"));
				}
	...
	@SideSubMenus(name="Create", priority=10, requiresPermission="Instance.ManagePermissions")
	public static void createGroupPage(@Nonnull final State st,@Nonnull final SafeMap values) {
		if ("Submit".equals(values.get("Submit"))) {
			st.form().add(Modules.run(st,"permissionsgroups.create",values));
		}
	...
	public static Response create(@Nonnull final State st,
		...
		try {
			PermissionsGroup.create(st,name);
	...
	public static void deleteForm(@Nonnull final State st,@Nonnull final SafeMap values) {
		if ("Submit".equals(values.get("Submit"))) {
			st.form().add(Modules.run(st,"permissionsgroups.delete",values));
		}
	...
	public static Response delete(@Nonnull final State st,
	                              ...
	                              @Arguments(name="permissionsgroup",
	                                         description="Permissions group to delete",
	...
	public static void editPermissions(@Nonnull final State st,@Nonnull final SafeMap values) {
		...
		table+="<tr bgcolor="+YELLOW+
		       "><th colspan=99>MEDIUM - permissions that edit stuff that has significant effect or can be hard to undo</th></tr>";
		table+=addPermissions(st,permissions.get(Permission.POWER.MEDIUM),pg,YELLOW);
		table+="<tr bgcolor="+RED+
		       "><th colspan=99>HIGH - permissions that can change global things, prevent the instance working, or destroy data</th></tr>";
