...
public abstract class Management {
    ...
    public static void viewFaction(State st, SafeMap values, CharacterGroup faction) throws SystemException {
        ...
        for (Char c:members) {
            ...
            if (faction.isAdmin(c)) { t.add(new Cell(new Colour("Red","Admin"))); } else { t.add(""); }
            if (c==factionowner) { t.add(new Cell(new Colour("Red","Leader"))); } else { t.add(""); }
    ...
    public static Response setOwner(State st,
        ...
        String oldownername=null; if (oldowner!=null) { oldownername=oldowner.getName(); }
        ...
        if (!ingroup) { return new ErrorResponse("New leader "+newowner.getName()+" must be in faction "+faction.getName()); }
        if (oldowner!=null) { oldowner.hudMessage(("You are no longer the faction leader for "+faction.getName())); }
    ...
    public static Response add(State st,
        ...
        if (existingfaction!=null && existingfaction.getOwner()==newmember) { return new ErrorResponse("Refusing to move character "+newmember.getName()+", they are currently faction leader of "+existingfaction.getName()+", you must manually eject them from that position"); }
        String success=""; if (existingfaction!=null) { existingfaction.removeMember(newmember); }
        if (!success.isEmpty()) { return new ErrorResponse("Failed to remove "+newmember.getName()+" from their existing faction "+faction.getName()+" : "+success); }
        try { faction.addMember(newmember); }
        catch (UserException e) { return new ErrorResponse("Failed to add "+newmember.getName()+" to "+faction.getName()+", they are probably in no faction now! - "+e.getMessage()); }
        String oldfactionname=null; if (existingfaction!=null) { oldfactionname=existingfaction.getName(); }
    ...
    public static Response remove(State st,
        ...
        if (existingfaction!=faction) { return new ErrorResponse("Can not remove "+member.getName()+" from "+faction.getName()+" because they are instead in "+existingfaction.getName()); }
        // refuse if they're the faction leader
    ...
    {
        if (!faction.getType().equals("Faction")) { return new ErrorResponse(faction.getName()+" is not a faction."); }
        if (!faction.hasMember(character)) { return new ErrorResponse(character.getName()+" is not a member of faction "+faction.getName()); }
        ...
        if (!ok) { return new ErrorResponse("You must be faction owner, or have Faction.Create permissions to set admin flags"); }
