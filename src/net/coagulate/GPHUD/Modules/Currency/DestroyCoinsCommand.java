...
public class DestroyCoinsCommand extends Command {
	...
	public DestroyCoinsCommand(final String name) {
		this.name=name;
	}
	...
	public List<Argument> getArguments() {
		...
		args.add(new Argument() {
			...
			@Override
			public String description() {
				return "Character to remove "+name+" from";
			}
		...
		args.add(new Argument() {
			...
			@Override
			public String description() {
				return "Ammount of "+name+" to remove";
			}
		...
		args.add(new Argument() {
			...
			@Override
			public String description() {
				return "Reason for currency destruction";
			}
	...
	protected Response execute(final State state,final Map<String,Object> arguments) {
		...
		if (target.isOnline()) {
			...
			JSONResponse.message(json,
			                     "[Admin:"+state.getAvatar().getName()+"] You lost "+currency.longTextForm(ammount)+
			                     " of "+currency.getName()+" : "+reason,
			                     target.getProtocol());
