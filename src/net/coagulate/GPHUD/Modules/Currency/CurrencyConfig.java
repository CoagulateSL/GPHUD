...
public class CurrencyConfig {
	...
	public static void configPage(@Nonnull final State st,final SafeMap values) {
		...
		f.add(new Paragraph(
				"Currencies are created and deleted in the Characters configuration page, add or remove CURRENCY type attributes"));
	...
	public static void configCurrency(@Nonnull final State st,final SafeMap values) {
		final Form f=st.form();
		f.noForm();
		...
		if (split.length==5) {
			...
			if (a.getInstance()!=st.getInstance()) {
				throw new SystemConsistencyException("Attribute instance/state instance mismatch");
			}
			if (a.getType()!=ATTRIBUTETYPE.CURRENCY) {
				throw new UserInputInvalidChoiceException(
						"Attribute "+a.getName()+" is not a currency, it's "+a.getType());
			}
	...
	private static void configCurrency(@Nonnull final State st,
		...
		f.add(new Paragraph(
				"This currency has a base coin of "+currency.getBaseCoinName()+" ("+currency.getBaseCoinNameShort()+
				")"));
		f.add(new Form(st,
		               true,
		...
		f.add(new Paragraph(
				currency.tradable()?"This currency <font color=green>can be traded</font> between characters":
				"This currency <font color=red>can not be traded</font> between characters"));
		f.add(new Form(st,
		               true,
		               "/GPHUD/Configuration/Currency/ToggleTradable",
		               "Toggle Tradable",
		               "currency",
		               currency.getName(),
		               "tradable",
		...
		for (final Coin coin: coins) {
			...
			t.add(new Form(st,
			               true,
			               "/GPHUD/Configuration/Currency/RemoveCoin",
			               "Delete Coin",
			               "currency",
			               currency.getName(),
			               "basevalue",
			               String.valueOf(coin.value)));
		}
		f.add(t);
		f.add(new Form(st,true,"/GPHUD/Configuration/Currency/AddCoin","Add Coin","currency",currency.getName()));
		f.add(new Paragraph(
				"<i>Note: Adding or removing coins does not change anyones balance, only how it is displayed</i>"));
	}
	
	@URLs(url="/configuration/currency/removecoin", requiresPermission="Currency.denominations")
	public static void removeCoinPage(@Nonnull final State st,@Nonnull final SafeMap values) {
	...
	public static Response removeCoin(@Nonnull final State state,
	                                  @Arguments(name="currency",
	                                             description="Currency to remove the base coin from",
	                                             type=ArgumentType.CURRENCY) @Nonnull final Currency currency,
	                                  @Arguments(name="basevalue",
	                                             description="Base value of the coin to delete",
	...
	@URLs(url="/configuration/currency/addcoin", requiresPermission="Currency.denominations")
	public static void addCoinPage(@Nonnull final State st,@Nonnull final SafeMap values) {
		Modules.simpleHtml(st,"Currency.addCoin",values);
	}
	
	@URLs(url="/configuration/currency/toggletradable", requiresPermission="Currency.Configure")
	public static void toggleTradablePage(@Nonnull final State st,@Nonnull final SafeMap values) {
	...
	public static Response toggleTradable(@Nonnull final State st,
	                                      @Arguments(name="currency",
	                                                 description="The currency to toggle",
	                                                 type=ArgumentType.CURRENCY) @Nonnull final Currency currency,
	                                      @Arguments(name="tradable",
	                                                 description="New tradable flag",
	...
	public static Response addCoin(@Nonnull final State state,
	                               @Arguments(name="currency",
	                                          description="Currency to add the coin to",
	                                          type=ArgumentType.CURRENCY) @Nonnull final Currency currency,
	                               @Arguments(name="basevalue",
	                                          description="Base value of the coin to add",
	                                          type=ArgumentType.INTEGER) @Nonnull final Integer basevalue,
	                               @Arguments(name="coinshortname",
	                                          description="Short name (symbol) of the coin",
	                                          type=ArgumentType.TEXT_ONELINE,
	                                          max=16) @Nonnull final String coinshortname,
	                               @Arguments(name="coinname",
	                                          description="Long name of the coin",
	                                          type=ArgumentType.TEXT_ONELINE,
	...
	@URLs(url="/Configuration/Currency/ChangeBaseCoinNames", requiresPermission="Currency.Denominations")
	public static void changeBaseCoinNames(@Nonnull final State st,@Nonnull final SafeMap values) {
	...
	public static Response setBaseCoinNames(@Nonnull final State state,
	                                        @Arguments(name="currency",
	                                                   description="Currency to alter",
	                                                   type=ArgumentType.CURRENCY) @Nonnull final Currency currency,
	                                        @Arguments(name="basecoinshortname",
	                                                   description="Short name of the coin",
	                                                   type=ArgumentType.TEXT_ONELINE,
	                                                   max=16) @Nonnull final String basecoinshortname,
	                                        @Arguments(name="basecoinname",
	                                                   description="Long name of the coin",
	                                                   type=ArgumentType.TEXT_ONELINE,
