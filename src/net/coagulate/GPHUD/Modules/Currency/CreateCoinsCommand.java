...
public class CreateCoinsCommand extends Command {
	...
	public CreateCoinsCommand(final String name) {
		this.name=name;
	}
	...
	public List<Argument> getArguments() {
		...
		args.add(new Argument() {
			...
			@Override
			public String description() {
				return "Character to give "+name+" to";
			}
		...
		args.add(new Argument() {
			...
			@Override
			public String description() {
				return "Ammount of "+name+" to give";
			}
		...
		args.add(new Argument() {
			...
			@Override
			public String description() {
				return "Reason for currency creation";
			}
	...
	protected Response execute(final State state,final Map<String,Object> arguments) {
		...
		if (target.isOnline()) {
			...
			JSONResponse.message(json,
			                     "[Admin:"+state.getAvatar().getName()+"] You gained "+currency.longTextForm(amount)+
