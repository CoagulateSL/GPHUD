...
public class EffectsCommands {
	...
	public static Response remove(@Nonnull final State st,
	                              @Argument.Arguments(name="target",
	                                                  description="Character to remove effect from",
	                                                  type=Argument.ArgumentType.CHARACTER) @Nonnull final Char target,
	                              @Argument.Arguments(name="effect",
	...
	@Command.Commands(description="Show effects applied to your character",
	                  ...
	                  // Characters tend to click this before being logged in, lets not error
	...
	public static Response show(@Nonnull final State st) {
		...
		if (effects.isEmpty()) {
			return new OKResponse("You have no effects applied");
		}
	...
	public static Response removeByMetadata(@Nonnull final State st,
	                                        @Argument.Arguments(name="target",
	                                                            description="Character to remove effect from",
	                                                            type=Argument.ArgumentType.CHARACTER) @Nonnull
	                                        final Char target,
		...
		for (final Effect effect: Effect.get(st,target)) {
			boolean zap=false;
			if (substring) {
				if (effect.getMetaData().contains(metaData)) {
					zap=true;
				}
			} else {
				if (effect.getMetaData().equals(metaData)) {
					zap=true;
				}
			}
			if (zap) {
	...
	private static Response orderedRemoval(@Nonnull final State st,
