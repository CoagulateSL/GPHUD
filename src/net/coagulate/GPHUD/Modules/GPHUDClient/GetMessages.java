...
public abstract class GetMessages {
	...
	public static Response acceptRejectMessage(@Nonnull final State st,
											   @Arguments(type=ArgumentType.CHOICE,
													   name="response", description="Accept or Reject the message",
	...
	private static Response processFactionInvite(@Nonnull final State st,
		...
		try {faction=CharacterGroup.get(j.getInt("to"));} catch (final NoDataException e) {
		...
		try {faction.getName();} catch (final NoDataException e) {
	...
	private static Response processFactionInviteResponse(@Nonnull final State st,
		...
		try {targetgroup=CharacterGroup.get(j.getInt("to"));} catch (final NoDataException e) {
		...
		if (targetgroup.getType()!=null) {
			...
			} catch (final NoDataException ignored) {} // dont care if existing faction somehow doesn't exist
