...
public class VerbEditor {
    ...
    public static void itemPage(@Nonnull final State st,
        ...
        table.add(selector(itemVerb.payload().optString("action","")));
        table.openRow("commandrow").add("Selected Command").add(DropDownList.getCommandsList(st,"command"));
        table.openRow("scriptrow").add("Selected Script").add(DropDownList.getScriptsList(st,"script"));
        f.add("<script> " +
                ...
                "} update(); </script>");
        table.openRow().add("Consumes Item").add(new CheckBox("consumesitem"));
