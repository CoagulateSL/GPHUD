...
public class VerbEditor {
	...
	public static void itemPage(@Nonnull final State st,@Nonnull final SafeMap values) {
		final boolean permitted=st.hasPermission("Items.EditVerbs");
		...
		table.openRow("commandrow")
		     .add("Selected Command")
		     .add(DropDownList.getCommandsList(st,"command").setValue(payload.optString("command","")));
		table.openRow("scriptrow")
		     .add("Selected Script")
		     .add(DropDownList.getScriptsList(st,"script").setValue(payload.optString("script","")));
		f.add("<script> "+"function update() { var action=document.getElementById(\"actiontype\").value; "+
		      "document.getElementById(\"scriptrow\").hidden=!(action==\"script\"); "+
		      "document.getElementById(\"commandrow\").hidden=!(action==\"command\"); "+"} update(); </script>");
		table.openRow()
		     .add("Consumes Item")
		     .add(new CheckBox("consumesitem").setValue(payload.optString("consumesitem","")));
		if (permitted) {
	...
	@URL.URLs(url="/configuration/items/deleteverb", requiresPermission="Items.DeleteVerb")
	...
	public static Response deleteVerb(@Nonnull final State st,
		...
		if (itemVerb==null) {
			return new ErrorResponse("There is no verb named '"+verb+"' for item "+item.getName());
		}
		itemVerb.delete();
		Audit.audit(true,
		            st,
		            ...
		            verb,
		            null,
		            "Deleted action "+verb+" from item "+item.getName());
