...
public class Interactions {
	...
	public static Response interactItemVerb(@Nonnull final State state,
	                                        ...
	                                        @Argument.Arguments(name="item",
	                                                            description="Item to interact with",
	                                                            type=Argument.ArgumentType.ITEM) @Nonnull
	                                        final Item item,
		...
		if ("Destroy".equalsIgnoreCase(verb)) {
			...
			json.put("arg0description",
			         "Ammount of "+item.getName()+" to destroy from "+inventory.getName()+"\n(You have "+count+
		...
		if ("Give To".equalsIgnoreCase(verb)) {
			...
			json.put("arg1description",
			         "Ammount of "+item.getName()+" to give from "+inventory.getName()+"\n(You have "+count+
			         ")\nUse -1 to GIVE ALL");
		...
		if ("Move To".equalsIgnoreCase(verb)) {
			...
			json.put("arg1name","quantity");
			json.put("arg1description",
			         "Ammount of "+item.getName()+" to move from "+inventory.getName()+"\n(You have "+count+
			         ")\nUse -1 to MOVE ALL");
			json.put("arg1type","TEXTBOX");
			json.put("args",2);
			return new JSONResponse(json);
	...
	public static Response destroyItem(@Nonnull final State state,
		...
		if (quantity<1) {
			return new ErrorResponse("You must destroy at least one item to do anything");
		}
		if (count<1) {
			return new ErrorResponse("Your inventory "+inventory.getName()+" does not contain any "+item.getName());
		}
		if (quantity>count) {
			return new ErrorResponse("You can not destroy "+quantity+" x "+item.getName()+" from "+inventory.getName()+
			                         " because it only contains "+count);
		}
		try {
			...
			Audit.audit(false,
			            ...
			            "Destroyed "+quantity+" "+item.getName()+" from "+inventory.getName());
			return new OKResponse(
					"Destroyed "+quantity+" x "+item.getName()+" from "+inventory.getName()+", "+newAmount+
					" remain in this inventory");
	...
	public static Response giveItem(@Nonnull final State state,
	                                ...
	                                @Argument.Arguments(name="target",
	                                                    ...
	                                                    type=Argument.ArgumentType.CHARACTER_NEAR) @Nonnull
	                                final Char target,
	                                @Argument.Arguments(name="quantity",
	                                                    type=Argument.ArgumentType.INTEGER,
		...
		if (quantity>count) {
			return new ErrorResponse("You can not give "+quantity+" x "+item.getName()+" from "+inventory.getName()+
			                         " because it only contains "+count);
		}
		try {
			...
			Audit.audit(false,
			            ...
			            "Recieve",
			            item.getName(),
			            String.valueOf(targetOldAmount),
			            String.valueOf(targetNewAmount),
			            "Received "+quantity+" "+item.getName()+" from "+state.getCharacter().getName()+"'s "+
			            inventory.getName()+", we had "+targetOldAmount+" and now have "+targetNewAmount);
			Audit.audit(false,
			            state,
			            Audit.OPERATOR.CHARACTER,
			            null,
			            ...
			            String.valueOf(count),
			            String.valueOf(newAmount),
			            "Gave "+quantity+" "+item.getName()+" from "+inventory.getName()+" to "+target.getName());
			final JSONResponse push=new JSONResponse(new JSONObject());
			push.message("You received "+quantity+" x "+item.getName()+" from "+state.getCharacter().getName()+" to "+
			             inventory.getName(),target.getProtocol());
			target.push(push);
			return new OKResponse(
					"Gave "+quantity+" x "+item.getName()+" from "+inventory.getName()+" to "+target.getName()+", "+
					newAmount+" remain in this inventory");
		} catch (final UserInventoryException e) {
			return new ErrorResponse(e.getLocalizedMessage());
		}
	}
	...
	public static Response moveItem(@Nonnull final State state,
	                                ...
	                                @Argument.Arguments(name="target",
	                                                    ...
	                                                    type=Argument.ArgumentType.INVENTORY) @Nonnull
		...
		if (quantity>count) {
			return new ErrorResponse("You can not move "+quantity+" x "+item.getName()+" from "+inventory.getName()+
			                         " because it only contains "+count);
		}
		if (!targetInv.allows(state,item)) {
			return new ErrorResponse("You can not move "+item.getName()+" to "+targetInv.getName()+
			                         " because it can not contain this item");
		}
		try {
			final int targetOldAmount=targetInv.count(item);
			final int targetNewAmount=targetInv.add(item,quantity,true);
			...
			Audit.audit(false,
			            ...
			            String.valueOf(targetOldAmount),
			            String.valueOf(targetNewAmount),
			            "Moved "+quantity+" "+item.getName()+" from "+inv.getName()+" to "+targetInv.getName());
			final JSONResponse push=new JSONResponse(new JSONObject());
			return new OKResponse(
					"Moved "+quantity+" x "+item.getName()+" from "+inv.getName()+" to "+targetInv.getName()+", "+
					newAmount+" remain in this inventory and "+targetNewAmount+" in the target inventory");
		} catch (final UserInventoryException e) {
			return new ErrorResponse(e.getLocalizedMessage());
		}
	}
