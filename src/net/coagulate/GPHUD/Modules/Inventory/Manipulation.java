...
public class Manipulation {
	...
	@Nonnull
	@Command.Commands(context=Command.Context.AVATAR, description="Add an item to an inventory")
	public static Response add(@Nonnull final State st,
	                           ...
	                           @Nonnull
	                           @Argument.Arguments(description="Item to add to inventory",
		...
		try {
			...
			Audit.audit(true,
			            ...
			            "Added "+qty+" "+item+" to inventory, totalling "+total);
			return new OKResponse(
					"Added "+qty+" "+item+" to "+character+"'s "+inventory+", changing total from "+oldValue+" to "+
					total);
	...
	private static Inventory preCheck(final State st,
	                                  final Attribute inventory,
	                                  final Char character,
	                                  final boolean checkAdmin) {
		// Attribute must be an inventory
		if (inventory.getType()!=Attribute.ATTRIBUTETYPE.INVENTORY) {
			throw new UserInputStateException(
					"Attribute "+inventory.getName()+" is of type "+inventory.getType()+" not INVENTORY");
		}
		// Attribute must belong to instance (!)
		if (inventory.getInstance()!=st.getInstance()) {
			throw new SystemImplementationException(
					"Attribute "+inventory+" is not from instance "+st.getInstanceString());
		}
		// check permission
		if (checkAdmin&&!st.hasPermission("Characters.Set"+inventory.getName())) {
			throw new UserAccessDeniedException("You do not have permission Characters.Set"+inventory.getName()+
			                                    " required to modify this characters inventory");
		}
	...
	@Nonnull
	@Command.Commands(context=Command.Context.AVATAR, description="Set the number of an item in an inventory")
	public static Response set(@Nonnull final State st,
		...
		Audit.audit(true,
		            ...
		            String.valueOf(oldValue),
		            String.valueOf(total),
		            "Set "+total+" x "+item.getName());
		return new OKResponse(
				"Set "+total+" "+item.getName()+" in "+character+"'s "+inventory.getName()+" (was "+oldValue+")");
	}
	
	@Nonnull
	@Command.Commands(context=Command.Context.CHARACTER, description="Reports on the contents of an inventory")
