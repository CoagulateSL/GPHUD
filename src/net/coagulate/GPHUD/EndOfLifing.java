...
public abstract class EndOfLifing {
	...
	public static boolean hasExpired(@Nullable final String version) {
		...
		try {
			...
			if (parts.length!=3) {
				SL.reportString("Weird version string received",
				                new SystemConsistencyException("Weird version string received"),
				                version);
			}
	...
	public static boolean hasExpired(final int major,final int minor,final int bugfix) {
	...
	// ---------- STATICS ----------
	@Nullable
	public static Date getEndOfLife(final int major,final int minor,final int bugfix) {
	...
	public static Float expiresIn(@Nullable final String version) {
		...
		try {
			...
			if (parts.length!=3) {
				SL.reportString("Weird version string received",
				                new SystemBadValueException("Weird version string received"),
				                version);
			}
	...
	public static Float expiresIn(final int major,final int minor,final int bugfix) {
		final Date endoflife=getEndOfLife(major,minor,bugfix);
		if (endoflife==null) {
			return null;
		}
		...
		@SuppressWarnings("UnnecessaryLocalVariable") final float daysexpiresin=
				expiresin/(60.0f*60.0f*24.0f); //convert to days
