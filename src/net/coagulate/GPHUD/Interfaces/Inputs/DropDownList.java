...
public class DropDownList extends Input {
	...
	private static final Cache<Instance,DropDownList> generatedCommandsCache   =
			Cache.getCache("gphud/gencmddropdown",CacheConfig.SHORT);
	
	public DropDownList(final String name) {
		this.name=name;
		add("");
	}
	private static final Cache<Instance,DropDownList> nongeneratedCommandsCache=
			Cache.getCache("gphud/nongencmddropdown",CacheConfig.SHORT);
	private static final Cache<Instance,DropDownList> permissionsCache=
			Cache.getCache("gphud/permissionsdropdown",CacheConfig.SHORT);
	private static final Cache<Instance,DropDownList> permissionsGroupsCache=
			Cache.getCache("gphud/permissionsgroupsdropdown",CacheConfig.SHORT);
	private static final Cache<Instance,DropDownList> charGroupsCache=
			Cache.getCache("gphud/chargroupsdropdown",CacheConfig.SHORT);
	private static final Cache<Instance,DropDownList> scriptsCache=
			Cache.getCache("gphud/scriptsdropdown",CacheConfig.SHORT);
	
	// ---------- STATICS ----------
	@Nonnull
	public static DropDownList getCommandsList(final State st,final String name) {
	...
	public static DropDownList getCommandsList(final State st,final String name,final boolean allowgenerated) {
		...
		if (allowgenerated) {
			targetCache=generatedCommandsCache;
		}
	...
	public void add(final String choice,final String label) {
		choices.put(choice,label);
	}
	
	public static DropDownList getPermissionsList(final State st,final String name) {
	...
	public static DropDownList getPermissionsGroups(final State st,final String name) {
	...
	public static DropDownList getCharacterGroups(final State st,final String name) {
	...
	public String asHtml(final State st,final boolean rich) {
		...
		if (submitOnChange) {
			r.append("onchange=\"this.form.submit()\" ");
		}
		if (!onchange.isBlank()) {
			r.append("onchange=\"").append(onchange).append("\" ");
		}
		if (!id.isBlank()) {
	...
	public void name(@Nonnull final String newName) {
		name=newName;
	}
	...
	@Nonnull
	public DropDownList javascriptOnChange(@Nonnull final String javascript) {
		onchange=javascript;
		return this;
	}
