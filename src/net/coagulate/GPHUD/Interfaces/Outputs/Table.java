...
public class Table implements Renderable {
	...
	@Nonnull
	public Table add(@Nonnull final Boolean b) {
	...
	public String asText(final State st) {
		...
		if (headerrow!=null) {
			res.append(headerrow.asText(st));
		}
	...
	public String asHtml(final State st,final boolean rich) {
		...
		if (nowrap) {
			s.append(" style=\"white-space: nowrap;\"");
		}
		s.append(">");
		if (headerrow!=null) {
			s.append(headerrow.asHtml(st,rich));
		}
		int rownum=0;
		for (final Row r: table) {
			if (r.isResetNumbering()) {
				rownum=0;
			}
