...
public class HTTPListener {
	...
	public static void initialise(final int port) {
		...
		try {
			...
			final SocketConfig reuse=SocketConfig.custom().setBacklogSize(100).setSoReuseAddress(true).build();
	...
	public static void shutdown() {
		...
		if (server!=null) {
			if (logging) {
				GPHUD.getLogger().info("Stopping listener gracefully");
			}
			server.shutdown(3,TimeUnit.SECONDS);
			if (logging) {
				GPHUD.getLogger().info("Terminating listener");
			}
			server.stop();
		}
	}
	...
	public static void shutdownNow() {
		...
		if (server==null) {
			GPHUD.getLogger().warning("Unable to stop server bootstrap as it never started?");
		}
