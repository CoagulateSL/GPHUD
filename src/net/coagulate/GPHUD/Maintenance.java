...
public class Maintenance extends Thread {
	...
	public static void purgeConnections() {
		try {
			final int purgecount=GPHUD.getDB()
			                          .dqinn("select count(*) from objects where lastrx<(UNIX_TIMESTAMP()-(60*60*24))");
			if (purgecount>0) {
	...
	public static void refreshCharacterURLs() {
		...
		if (results.notEmpty()) {
			...
			for (final ResultsRow r: results) {
				...
				final Transmission t=new PingTransmission(Char.get(r.getInt("characterid")),
				                                          ping,
				                                          r.getStringNullable("url")
				);
				t.start();
