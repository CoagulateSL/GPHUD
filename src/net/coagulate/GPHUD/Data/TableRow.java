...
public abstract class TableRow extends net.coagulate.Core.Database.TableRow implements Renderable,Comparable {
    ...
    public int resolveToID(State st,String s,boolean instancelocal) {
        ...
        try {
            ...
            if (id>0) { return id; }
        } catch (NoDataException e) {if (debug) { System.out.println("NO DATA");} }
        catch (TooMuchDataException e) { GPHUD.getLogger().warning("Multiple matches searching for "+s+" in "+this.getClass().toString()); }
        return 0;
    }
    ...
    public void kvcheck() {
        if (getKVTable()==null || getKVIdField()==null) { throw new SystemException("DBObject "+this.getClass().getName()+" does not support KV mappings"); }
    }
    ...
    private static class CacheElement {
        ...
        public int expires;
        public CacheElement(Object element,int expires) {this.element=element; this.expires=expires;}
    }
    
    Object cacheGet(String key) throws CacheMiss {
        if (!cache.containsKey(key)) { throw new CacheMiss(); }
        CacheElement ele=cache.get(key);
        if (ele.expires<getUnixTime()) { cache.remove(key); throw new CacheMiss(); }
        return ele.element;
    }
