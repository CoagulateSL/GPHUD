...
public class Landmark extends TableRow {
	...
	public static Set<Landmark> getAll(@Nonnull final Instance instance) {
		...
		for (final ResultsRow row: db().dq(
				"select landmarks.id as id from landmarks,regions where landmarks.regionid=regions.regionid and regions.instanceid=?",
				instance.getId())) {
	 ...
	 */
	@Nullable
	public static Landmark find(@Nonnull final State st,@Nonnull final String name) {
	 ...
	 */
	@Nullable
	public static Landmark find(@Nonnull final Instance instance,@Nonnull final String name) {
		try {
			final int id=db().dqiNotNull(
					"select landmarks.id from landmarks,regions where landmarks.regionid=regions.regionid and regions.instanceid=? and landmarks.name like"+
					" ?",
					instance.getId(),
					name);
	...
	public static void create(@Nonnull final Region region,
		...
		db().d("insert into landmarks(regionid,name,x,y,z,lookatx,lookaty,lookatz) values(?,?,?,?,?,?,?,?)",
		       region.getId(),
		       name,
		       x,
		       y,
	 ...
	 */
	public static void obliterate(@Nonnull final Instance instance,@Nonnull final String name) {
