...
public class Message extends TableRow {
	 ...
	 * @param json         Message
	 */
	public static void queueDays(@Nonnull final Char c,final int lifespandays,@Nonnull final JSONObject json) {
	 ...
	 * @param json          Message
	 */
	public static void queueHours(@Nonnull final Char c,final int lifespanhours,@Nonnull final JSONObject json) {
	...
	static void add(@Nonnull final Char targetchar,final int expires,@Nonnull final JSONObject message) {
		db().d("insert into messages(characterid,expires,json) values(?,?,?)",
		       targetchar.getId(),
		       expires,
		       message.toString());
	 ...
	 */
	public static void pushMessageCount(final Char character) {
		character.push("messagecount",Integer.toString(count(character)));
	}
	...
	public static Message getNextMessage(@Nonnull final Char c) {
		try {
			final int id=db().dqiNotNull(
					"select messageid from messages where characterid=? order by expires  limit 0,1",
					c.getId());
	...
	protected Message(final int id) {
		super(id);
	}
	 ...
	 */
	public static void pushMessageCount(final State state) {
		state.getCharacter().push("messagecount",Integer.toString(count(state.getCharacter())));
	}
