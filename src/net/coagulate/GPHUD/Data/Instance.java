...
/*    protected void delete() {
    ...
    public void updateStatus() {
        ...
        for (Region r:getOurRegions()) {
            ...
            if (url==null || url.isEmpty()) {
                ...
                if (canStatus("admins")) { broadcastAdmins(null, "SYSTEM : Alert, region server for '"+r.getName()+"' is not connected to GPHUD Server."); }
                if (level<2) { statuscolor="<1.0,0.5,0.5>"; level=2; }
            } else {
                if ((getUnixTime()-urllast)>(15*60)) {
                    newstatus+="*STALLED*";
                    if (canStatus("admins")) { broadcastAdmins(null, "SYSTEM : Alert, region server for '"+r.getName()+"' is not communicating (STALLED / CRASHED)??."); }
                    if (level<2) { statuscolor="<1.0,0.5,0.5>"; level=2; }
                }
                newstatus+=visitors;
    ...
    private boolean canStatus(String url) {
        ...
        if (laststatused.containsKey(url)) {
            ...
            if (url.equals("admins")) {  // bodge, admins get 5 minute pesterings :P
                if ((now-last)>300) { laststatused.put(url,now); return true; }
                return false;
            }
            if ((now-last)>30) { laststatused.put(url,now); return true; }
            return false;
        }
    ...
    public List<CharacterSummary> getCharacterSummary(State st) {
        ...
        if (sortby.startsWith("-")) { reverse=true; sortby=sortby.substring(1); }
        //System.out.println(sortby+" "+reverse);
        ...
        for (ResultsRow r:dq("select * from characters where instanceid=?",getId())) {
            ...
            cr.online=false; if (r.getString("url")!=null && !r.getString("url").isEmpty()) { cr.online=true; }
            idmap.put(charid, cr);
        }
        ...
        for (ResultsRow r:dq("select characterid,starttime from visits where endtime is null and starttime>?",UnixTime.getUnixTime()-(Experience.getCycle(st)))) {
            ...
            if (idmap.containsKey(id)) { idmap.get(id).recentvisits=idmap.get(id).recentvisits+add; idmap.get(id).totalvisits=idmap.get(id).totalvisits+add; }
        }        
        ...
        if (sortby.equals("name") || sortby.equals("owner")) {
            ...
            if (reverse) { Collections.sort(sortedkeys,Collections.reverseOrder()); }  else { Collections.sort(sortedkeys); } 
            ...
            for (CharacterSummary cs:idmap.values()) {
                ...
                if (sortby.equals("visit time (last "+Experience.getCycleLabel(st).toLowerCase()+")")) { value=cs.recentvisits; }
            ...
            sortedkeys.addAll(sorted.keySet());
            if (!reverse) { Collections.sort(sortedkeys,Collections.reverseOrder()); }  else { Collections.sort(sortedkeys); } // note reverse is reversed for numbers
    ...
    public void pushConveyances() {
        ...
        for (Char c:Char.getActive(this)) {
            ...
            if (debug) { System.out.println("PUSH CONSIDERATION : "+simulated.getCharacter()+" - "+payload.toString()); }
            if (!payload.keySet().isEmpty()) {
                Region reg=c.getRegion();
                if (reg!=null) {
                    if (debug) { System.out.println("Adding"); }
                    if (!buffer.containsKey(reg)) { if (debug) { System.out.println("created buffer"); } buffer.put(reg,new JSONObject().put("incommand","disseminate")); }
                    buffer.get(reg).put(c.getPlayedBy().getUUID(),payload.toString());
                    if (buffer.get(reg).toString().length()>(3*1024)) {
                        if (debug) { System.out.println("Size cautionary push: "+reg+" : "+buffer.get(reg).toString()); }
