...
public class Script extends TableRow {
	...
	private static final Cache<Instance,Set<Script>> scriptCache=
			Cache.getCache("gphud/scriptlist",CacheConfig.OPERATIONAL_CONFIG);
	...
	public static Table getTable(@Nonnull final State state,final boolean canDelete) {
		...
		final Results rows=db().dq(
				"select id,name,sourceversion,bytecodeversion,alias from scripts where instanceid=? order by name asc",
				instance.getId());
		...
		for (final ResultsRow row: rows) {
			o.openRow();
			o.add("<a href=\"/GPHUD/configuration/scripting/edit/"+row.getIntNullable("id")+"\">"+
			      row.getStringNullable("name")+"</a>");
			...
			o.add(alias);
			if (canDelete) {
				o.add(new Form(state,
				               true,
				               "/GPHUD/configuration/scripting/delete",
				               "Delete",
				               "scriptname",
				               row.getStringNullable("name")));
			}
		}
		return o;
	}
	 ...
	 */
	public static void create(@Nonnull final State st,@Nonnull final String scriptname) {
		final int existing=db().dqiNotNull("select count(*) from scripts where name like ? and instanceid=?",
		                                   scriptname,
		                                   st.getInstance().getId());
		if (existing>0) {
			throw new UserInputDuplicateValueException("script with that name already exists");
		}
	 ...
	 */
	@Nullable
	public static Script findNullable(@Nonnull final State st,@Nonnull final String scriptname) {
		try {
			return find(st,scriptname);
		} catch (@Nonnull final UserInputLookupFailureException e) {
			return null;
		}
	}
	 ...
	 */
	@Nonnull
	public static Script find(@Nonnull final State st,@Nonnull final String scriptname) {
		try {
			final int id=db().dqiNotNull("select id from scripts where instanceid=? and name like ?",
			                             st.getInstance().getId(),
			                             scriptname);
			return new Script(id);
		} catch (final NoDataException ignore) {
			try {
				final int id=db().dqiNotNull("select id from scripts where instanceid=? and alias like ?",
				                             st.getInstance().getId(),
				                             scriptname);
				return new Script(id);
			} catch (final NoDataException nde) {
				throw new UserInputLookupFailureException("Script by name/alias "+scriptname+" does not exist",
				                                          nde,
				                                          true);
	 ...
	 */
	@Nonnull
	public static DropDownList getList(@Nonnull final State st,@Nonnull final String listname) {
...
@Nonnull
	...
	public String getLinkTarget() {
		return "/GPHUD/configuration/scripting/edit/"+getId();
	}
...
/**
	 ...
	 */
	public void setBytecode(@Nonnull final Byte[] toByteCode,final int version,final int compilerversion) {
		validate();
		d("update scripts set bytecode=?, bytecodeversion=?, compilerversion=? where id=?",
		  toByteCode,
		  version,
		  compilerversion,
		  getId());
		if (Config.getDevelopment()) {
			...
			if (compareto.length!=toByteCode.length) {
				throw new SystemImplementationException(
						"Length mismatch, wrote "+toByteCode.length+" and read "+compareto.length);
			}
			for (int i=0;i<compareto.length;i++) {
				if (compareto[i]!=toByteCode[i]) {
					throw new SystemImplementationException(
							"Difference at "+i+" - we wrote "+toByteCode[i]+" and read "+compareto[i]);
				}
			}
		}
	}
