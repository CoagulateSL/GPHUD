...
public class CharacterSet {
	...
	private void checks() {
		if (set.getType()!=SET) {
			throw new SystemImplementationException(
					"Accessing a set of a non SET attribute type "+set.getName()+" is "+set.getType());
		}
	...
	public static Set<Attribute> getAll(@Nonnull final State state) {
	...
	public int countElements() {
		return db().dqiNotNull("select count(*) from charactersets where characterid=? and attributeid=?",
		                       character.getId(),
		                       set.getId());
	}
	...
	public int countTotal() {
		final Integer result=db().dqi("select sum(qty) from charactersets where characterid=? and attributeid=?",
		                              character.getId(),
		                              set.getId());
		if (result==null) {
			return 0;
	...
	public int count(@Nonnull final String element) {
		final Results rows=db().dq("select qty from charactersets where characterid=? and attributeid=? and element=?",
		                           character.getId(),
		                           set.getId(),
		                           element);
		if (rows.empty()) {
	...
	public void set(@Nonnull final String element,final int setTo) {
		...
		db().d("insert into charactersets(characterid,attributeid,element,qty) values(?,?,?,?) on duplicate key update qty=?",
		       character.getId(),
		       set.getId(),
		       element,
		       setTo,
	...
	public void delete(@Nonnull final String element) {
		db().d("delete from charactersets where characterid=? and attributeid=? and element=?",
		       character.getId(),
		       set.getId(),
		       element);
	}
	...
	public Set<String> list() {
		...
		for (final ResultsRow row: db().dq("select element from charactersets where characterid=? and attributeid=?",
		                                   character.getId(),
		                                   set.getId())) {
	...
	public Map<String,Integer> elements() {
		...
		for (final ResultsRow row: db().dq("select element,qty from charactersets where characterid=? and attributeid=?",
		                                   character.getId(),
		                                   set.getId())) {
