...
public class TestCurrency extends TestTools {
	...
	private static void createCurrency(final TestFramework t) {
		...
		final Currency gold=getCurrency(t);
	...
	public static TestFrameworkPrototype.TestOutput testCurrencyAfterDelete(final TestFramework t) {
		createCurrency(t);
		final Currency currency=getCurrency(t);
		deleteCurrency(t);
		Currency postDelete=null;
		try { postDelete=getCurrency(t); } catch (final Exception ignore) {}
	...
	public static TestFrameworkPrototype.TestOutput testCurrencyAttributeAfterDelete(final TestFramework t) {
		createCurrency(t);
		final Currency currency=getCurrency(t);
		final Attribute attr=Attribute.find(t.primaryHUD.character.getInstance(),"gold");
		deleteCurrency(t);
		Attribute postDelete=null;
		try { postDelete=Attribute.find(t.primaryHUD.character.getInstance(),"gold"); } catch (final Exception ignore) {}
	...
	public static TestFrameworkPrototype.TestOutput testCurrencyFormatting(final TestFramework t) {
		createCurrency(t);
		final Currency currency=getCurrency(t);
		final Pool pool=currency.getPool(t.primaryHUD.state);
		...
		final int sum=pool.sum(t.primaryHUD.state);
		final String shortform=currency.shortTextForm(sum);
		final String longform=currency.longTextForm(sum);
		deleteCurrency(t);
