...
public class Effect extends TableRow {
	...
	public static Effect get(@Nonnull final State st,@Nonnull final String name) {
		...
		if (effect==null) {
			throw new UserInputLookupFailureException("There is no effect named "+name);
		}
		return effect;
	}
	...
	public static Effect find(@Nonnull final Instance instance,final String name) {
		final Results matches=
				db().dq("select id from effects where instanceid=? and name like ?",instance.getId(),name);
		if (matches.empty()) {
			return null;
		}
		if (matches.size()>1) {
			throw new TooMuchDataException(
					"Name "+name+" in instance "+instance.getId()+" matched "+matches.size()+" results");
		}
	...
	public static void create(@Nonnull final State st,
		...
		if (Effect.getNullable(st,name)!=null) {
			throw new UserInputDuplicateValueException("There is already an effect named "+name);
		}
		if (metaData==null) {
			metaData="";
		}
	...
	public static void expirationCheck(@Nonnull final State st,@Nonnull final Char character) {
		if (st.expirationChecked) {
			return;
		}
		for (final ResultsRow row: db().dq("select effectid from effectsapplications where characterid=? and expires<?",
		                                   character.getId(),
	...
	public static Set<Effect> get(@Nonnull final State st,@Nonnull final Char character) {
		return effectCache.get(character,()->{
			...
			for (final ResultsRow row: db().dq(
					"select effectid from effectsapplications where characterid=? and expires>=?",
					character.getId(),
					UnixTime.getUnixTime())) {
		...
		for (final Entry<Integer,Effect> entry: byduration.entrySet()) {
			if (i<4) {
				...
				if (texture==null||texture.isEmpty()) {
					texture="b39860d0-8c5c-5d51-9dbf-3ef55dafe8a4";
				}
	...
	static void wipeKV(@Nonnull final Instance instance,final String key) {
		...
		db().d("delete from "+kvtable+" using "+kvtable+","+maintable+" where "+kvtable+".k like ? and "+kvtable+"."+
		       kvidcolumn+"="+maintable+"."+maintableidcolumn+" and "+maintable+".instanceid=?",key,instance.getId());
	}
	...
	public boolean unapply(@Nonnull final State st,@Nonnull final Char character,final boolean audit) {
		if (dqinn("select count(*) from effectsapplications where characterid=? and effectid=?",
		          character.getId(),
		          getId())==0) {
			return false;
		}
	...
	public boolean apply(@Nonnull final State st,
		...
		validate(st);
		if (seconds<=0) {
			throw new UserInputInvalidChoiceException("Number of seconds to apply effect must be greater than zero",
			                                          true);
		}
		final int expires=UnixTime.getUnixTime()+seconds;
		// any existing?
		if (dqinn("select count(*) from effectsapplications where effectid=? and characterid=? and expires>=?",
		          getId(),
		          target.getId(),
		          expires)>0) {
		...
		Audit.audit(true,
		            ...
		            "",
		            getName(),
		            "Applied effect "+getName()+" for "+UnixTime.duration(seconds));
	...
	public boolean remove(@Nonnull final State st,@Nonnull final Char target,final boolean administrative) {
		...
		if (administrative) {
			Audit.audit(true,
			            ...
			            getName(),
			            "",
			            "Administratively removed effect "+getName());
		} else {
			Audit.audit(true,
			            st,
			            OPERATOR.CHARACTER,
	...
	public int remains(@Nonnull final Char character) {
		final Results set=dq("select expires from effectsapplications where effectid=? and characterid=?",
		                     getId(),
		                     character.getId());
		if (set.size()==0) {
			return -1;
		...
		if (set.size()>1) {
			throw new SystemConsistencyException(getName()+" has "+set.size()+" applications to "+character);
		}
