...
public class Attribute extends TableRow {
	...
	private static final Cache<Instance,Set<Attribute>> attributeSetCache=
			Cache.getCache("GPHUD/attributeSet",CacheConfig.OPERATIONAL_CONFIG);
	...
	protected Attribute(final int id) {
		super(id);
	}
	 ...
	 */
	@Nonnull
	public static Attribute findGroup(@Nonnull final Instance instance,@Nonnull final String groupType) {
		try {
			final int id=db().dqiNotNull(
					"select attributeid from attributes where instanceid=? and attributetype='GROUP' and grouptype=?",
					instance.getId(),
					groupType);
			return get(id);
		} catch (@Nonnull final NoDataException e) {
			throw new UserInputLookupFailureException(
					"Unable to find an attribute representing a group of type "+groupType,e);
		}
	}
	 ...
	 */
	@Nonnull
	public static Set<Attribute> getAttributes(@Nonnull final State st) {
		return getAttributes(st.getInstance());
	}
	...
	public static Set<Attribute> getAttributes(@Nonnull final Instance instance) {
		return attributeSetCache.get(instance,()->{
			...
			for (final ResultsRow r: db().dq("select attributeid from attributes where instanceid=?",
			                                 instance.getId())) {
	 ...
	 */
	@Nullable
	public static Attribute findNullable(@Nonnull final State st,final String name) {
	...
	public void setRequired(final boolean required) {
		...
		requiredCache.set(this,required);
	}
	...
	public String getDefaultValue() {
		return getStringNullable("defaultvalue");
	}
	...
	public String getCharacterValue(@Nonnull final State st) {
		...
		if (isKV()) {
			return st.getKV("Characters."+getName()).value();
		}
	...
	public String getCharacterValueDescription(@Nonnull final State st) {
		if (isKV()) {
			return st.getKV("Characters."+getName()).path();
		}
		final ATTRIBUTETYPE attributetype=getType();
		if (attributetype==EXPERIENCE) {
			final GenericXP xp=new GenericXP(getName());
	...
	public boolean isKV() {
		...
		if (def==INTEGER||def==FLOAT||def==TEXT||def==COLOR) {
			return true;
		}
		if (def==POOL||def==GROUP||def==EXPERIENCE||def==CURRENCY||def==SET||def==INVENTORY) {
			return false;
		}
	...
	public KV.KVTYPE getKVType() {
		final ATTRIBUTETYPE def=getType();
		if (def==INTEGER) {
			return KV.KVTYPE.INTEGER;
		}
		if (def==FLOAT) {
			return KV.KVTYPE.FLOAT;
		}
		if (def==TEXT) {
			return KV.KVTYPE.TEXT;
		}
		if (def==COLOR) {
			return KV.KVTYPE.COLOR;
		}
	...
	public ATTRIBUTETYPE getType() {
		return attributeTypeCache.get(this,()->fromString(getString("attributetype")));
	}
	...
	public KV.KVHIERARCHY getKVHierarchy() {
		final ATTRIBUTETYPE def=getType();
		if (def==INTEGER) {
			return KV.KVHIERARCHY.CUMULATIVE;
		}
		if (def==FLOAT) {
			return KV.KVHIERARCHY.CUMULATIVE;
		}
		if (def==TEXT) {
			return KV.KVHIERARCHY.DELEGATING;
		}
		if (def==COLOR) {
			return KV.KVHIERARCHY.DELEGATING;
		}
	...
	public void delete(final State st) {
		...
		if (instance!=st.getInstance()) {
			throw new SystemConsistencyException(
					"State instance / attribute instance mismatch during DELETE of all things");
		}
		final ATTRIBUTETYPE type=getType();
		if (type==TEXT||type==FLOAT||type==INTEGER||type==COLOR) {
	...
	public void templatable(final State st,final boolean newValue) {
		...
		Audit.audit(false,
		            ...
		            "Set",
		            getName()+"/Templatable",
		            String.valueOf(templatable()),
		            String.valueOf(newValue),
		            "Set templatable to "+newValue);
		set("templatable",newValue);
		templatableCache.set(this,newValue);
	}
	
	public enum ATTRIBUTETYPE {
		TEXT,FLOAT,INTEGER,GROUP,POOL,COLOR,EXPERIENCE,CURRENCY,SET,INVENTORY
	}
}
