...
public class Zone extends TableRow {
	 ...
	 */
	public static void create(@Nonnull final Instance instance,@Nonnull final String name) {
	 ...
	 */
	@Nullable
	public static Zone findNullable(@Nonnull final Instance instance,@Nonnull final String name) {
		try {
			final int zoneid=db().dqiNotNull("select zoneid from zones where name like ? and instanceid=?",
			                                 name,
			                                 instance.getId());
			return get(zoneid);
	...
	public void validate(@Nonnull final State st) {
		...
		if (st.getInstance()!=getInstance()) {
			throw new SystemConsistencyException("Zone / State Instance mismatch");
		}
	}
	...
	public void broadcastMessage(@Nonnull final String message) {
		...
		GPHUD.getLogger()
		     .info("Sending broadcast to zone "+getName()+" in instance "+getInstance().getName()+" - "+message);
