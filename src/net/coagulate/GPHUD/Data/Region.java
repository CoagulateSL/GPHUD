...
public class Region extends TableRow {
    ...
    public String getURL(boolean permitnull) throws UserException {
        ...
        if (url==null) { if (permitnull) { return null; } else { throw new UserException("This region has no callback URL"); } }
        return url;
    }
    /** Gets the URL associated with this region's server
     * 
    ...
    public void setURL(String url) {
        ...
        if (oldurl!=null && oldurl.equals(url)) {
            if ((now-getURLLast())>60) { d("update regions set urllast=?,authnode=? where regionid=?",now,Interface.getNode(),getId()); }
            return;
        }
    ...
    private String formatVersion(Integer version,Integer datetime,boolean html) {
        ...
        if (version==null) { v+="v??? "; } else { v=v+"v"+(version/10000)+"."+((version/100)%100)+"."+(version%100)+" "; }
    ...
    public String getOnlineStatus(String timezone) {
        ...
        if (getURL(true)==null || getURL(true).isEmpty()) { return "OFFLINE for "+duration(getUnixTime()-urllast); }
        String authnode=getAuthNode();
        if ((getUnixTime()-urllast)>(15*60)) { return "STALLED at "+fromUnixTime(urllast,timezone)+" via server "+authnode; }
