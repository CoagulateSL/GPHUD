...
public class AuditTable extends PagedSQL {
	 ...
	 */
	public AuditTable(final State state,final String prefix,final SafeMap parameters) {
		super(state,prefix,parameters);
		timezone=state.getAvatar().getTimeZone();
	}
	
	public AuditTable(final State state,final String prefix,final SafeMap parameters,final Char forchar) {
		super(state,prefix,parameters);
		timezone=state.getAvatar().getTimeZone();
	...
	protected String formatRow(@Nonnull final State state,@Nonnull final ResultsRow row) {
		...
		ret+="<td>";
		if (!(sourcename.isEmpty()&&sourceowner.isEmpty()&&sourcedev.isEmpty()&&sourceregion.isEmpty()&&
		      sourceloc.isEmpty())) {
	...
	protected int getRowCount() {
		return db().dqiNotNull("select count(*) from audit where instanceid="+instance.getId()+" "+getAdditionalWhere()+
	...
	protected Results runQuery() {
		return db().dq("select * from audit where instanceid="+instance.getId()+" "+getAdditionalWhere()+
		               " order by timedate desc "+getSQLLimit());
	}
