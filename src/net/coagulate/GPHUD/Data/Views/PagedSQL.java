...
public abstract class PagedSQL implements Renderable {
	...
	protected PagedSQL(@Nonnull final State state,@Nonnull final String prefix,@Nonnull final SafeMap parameters) {
	...
	protected static String formatavatar(@Nonnull final NameCache cache,@Nullable final Integer avatarid) {
		if (avatarid!=null) {
			return cache.lookup(User.get(avatarid));
		}
		return "";
	}
	
	protected static String formatchar(@Nonnull final NameCache cache,@Nullable final Integer charid) {
		if (charid!=null) {
			return cache.lookup(Char.get(charid));
		}
		return "";
	}
	
	protected static String formatregion(@Nonnull final NameCache cache,@Nullable final Integer charid) {
		if (charid!=null) {
			return cache.lookup(Region.get(charid,true));
		}
		return "";
	}
	...
	@Nonnull
	protected static Renderable notate(@Nonnull final String s,final int size) {
		if (s.length()>size) {
	...
	public String asHtml(final State st,final boolean rich) {
		if (parameters.containsKey(prefix+"-page-default")) {
			page=Integer.parseInt(parameters.get(prefix+"-page-default"));
		}
		if (parameters.containsKey(prefix+"-page")) {
			page=Integer.parseInt(parameters.get(prefix+"-page"));
		}
		if (parameters.containsKey(prefix+"-search")) {
			searchtext=parameters.get(prefix+"-search");
		}
	...
	private String getPageRow() {
		...
		r+="<button type=submit name="+prefix+"-submit value="+prefix+"-submit>Search:</button><input type=text name="+
		   prefix+"-search value=\""+searchtext+"\">";
		r+="&nbsp;&nbsp;&nbsp;";
		...
		r+="&nbsp;&nbsp;&nbsp;";
		r+="<button "+((page+1)>=pagecount?"disabled":"")+" type=submit name="+prefix+"-page value="+(page+1)+
		   ">&gt;&gt;</button>";
		r+="</td></tr>";
		return r;
	}
	...
	@Nonnull
	protected abstract String formatRow(@Nonnull final State state,@Nonnull final ResultsRow row);
