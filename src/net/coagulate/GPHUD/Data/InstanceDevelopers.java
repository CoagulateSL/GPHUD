...
public class InstanceDevelopers extends Table {
	...
	public static void accounting(@Nonnull final Instance instance,
	                              ...
	                              final int queries,
	                              final int bytes) {
		db().d("update instancedevelopers set queries=queries+?,bytes=bytes+? where instanceid=? and developerid=?",queries,bytes,instance.getId(),developer.getId());
	}
	...
	public static void deAuthorise(@Nonnull final State state,
	                               ...
	                               @Nonnull final User developer) {
		if (!isDeveloper(instance,developer)) { return; }
		db().d("delete from instancedevelopers where instanceid=? and developerid=?",instance.getId(),developer.getId());
		Audit.audit(true,state,OPERATOR.AVATAR,null,null,"Delete","External",developer.getName(),"","User deauthorised developer "+developer.getName());
	}
	...
	public static void authorise(@Nonnull final State state,
		...
		Audit.audit(true,state,OPERATOR.AVATAR,null,null,"Add","External","",developer.getName(),"User authorised external developer access to "+developer.getName());
	}
